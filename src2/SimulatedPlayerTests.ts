...
GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_puffer", (test) => {
  const spawnLoc = { x: 1, y: 5, z: 1 };
  const landLoc = { x: 1, y: 2, z: 1 };
...
GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_other", (test) => {
  const spawnLoc = { x: 1, y: 5, z: 1 };
  const landLoc = { x: 1, y: 2, z: 1 };
  ...
  GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_custom", (test) => {
    const spawnLoc = { x: 1, y: 5, z: 1 };
    const landLoc = { x: 1, y: 2, z: 1 };
    const playerName = world.getDynamicProperty("customSimulatedPlayerName");
    const player = test.spawnSimulatedPlayer(spawnLoc, playerName);
    test.assertEntityPresent("player", spawnLoc);/*
    test.assert(player.nameTag === playerName, "Unexpected name tag");*/
    test
    ...
    .thenExecuteAfter(0, () => {if (player.hasTag("customandexdbSimulatedPlayer:remove")) { test.removeSimulatedPlayer()} else { if (player.hasTag("customandexdbSimulatedPlayer:remove")) { player.respawn()}  else { if (player.hasTag("customandexdbSimulatedPlayer:disconnect")) { player.disconnect()}   else { if (player.hasTag("customandexdbSimulatedPlayer:fly")) { player.fly()}   else { if (player.hasTag("customandexdbSimulatedPlayer:stopFlying")) { player.stopFlying()} } } } }})
    .thenExecuteAfter(0, () => {
      test.assertEntityPresent("player", spawnLoc, false);
    })
    .thenExecuteAfter(2147483647, () => {
...
GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_Andexter8", (test) => {
  const spawnLoc = { x: 1, y: 5, z: 1 };
  const landLoc = { x: 1, y: 2, z: 1 };
...
GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_SashaS555", (test) => {
  ...
  GameTest.register("SimulatedPlayerTests", "spawn_simulated_player_Herobrine", (test) => {
    const spawnLoc = { x: 1, y: 5, z: 1 };
    const landLoc = { x: 1, y: 2, z: 1 };
...
GameTest.register("SimulatedPlayerTests", "set_item", (test) => {
  ...
  test.spawn("blaze", new BlockLocationIterator(1, 2, 2));
...
GameTest.register("SimulatedPlayerTests", "interact_with_entity", (test) => {
  ...
  const minecart = test.spawn("minecart", new BlockLocationIterator(1, 2, 1));
  player.interactWithEntity(minecart);
  test.succeedWhenEntityPresent("minecraft:player", new BlockLocationIterator(1, 3, 1));
}).tag(GameTest.Tags.suiteDefault);

GameTest.register("SimulatedPlayerTests", "destroy_block", (test) => {
  const player = test.spawnSimulatedPlayer(new BlockLocationIterator(0, 2, 0));
  const fenceLoc = new BlockLocationIterator(1, 2, 0);
  const chestLoc = new BlockLocationIterator(2, 2, 0);
  const ironOreLoc = new BlockLocationIterator(0, 2, 1);
  const planksLoc = new BlockLocationIterator(1, 2, 1);
...
GameTest.register("SimulatedPlayerTests", "stop_destroying_block", (test) => {
  const ironOreLoc = new BlockLocationIterator(1, 2, 1);
...
GameTest.register("SimulatedPlayerTests", "one_tick", (test) => {
  for (let i = 0; i < 3; i++) {
    test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 0));
  }
...
GameTest.registerAsync("SimulatedPlayerTests", "run_command_after_spawn", async (test) => {
  const spawnLoc = new BlockLocationIterator(1, 2, 2);
...
GameTest.register("SimulatedPlayerTests", "sneaking", (test) => {
  const player = test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 0));
  const goalLoc = new BlockLocationIterator(1, 2, 3);
...
GameTest.registerAsync("SimulatedPlayerTests", "player_join_leave_events", async (test) => {
  ...
  let simPlayer = test.spawnSimulatedPlayer(new BlockLocationIterator(0, 2, 0), thePlayerName);
  await test.idle(1);
...
GameTest.registerAsync("SimulatedPlayerTests", "player_uses_correct_item_from_updated_slot", async (test) => {
  const player = test.spawnSimulatedPlayer(new BlockLocationIterator(0, 2, 0));
  const blockLoc = new BlockLocationIterator(2, 1, 1);
  const dirt = new ItemStack(MinecraftItemTypes.dirt, 1);
