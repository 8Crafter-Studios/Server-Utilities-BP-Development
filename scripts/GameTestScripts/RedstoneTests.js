...
import { BlockLocationIterator, MinecraftBlockTypes, MinecraftItemTypes, BlockProperties, world } from "@minecraft/server";
...
GameTest.register("RedstoneTests", "itemframe_override", (test) => {
    const itemFrameTest = new BlockLocationIterator(3, 2, 5);
    const itemFrameOverrideNoTest = new BlockLocationIterator(3, 2, 10);
    const lever = new BlockLocationIterator(1, 2, 0);
    const leverOverrideTest = new BlockLocationIterator(1, 2, 13);
...
GameTest.register("RedstoneTests", "itemframe_override_bedrock", (test) => {
    const itemFrameTest = new BlockLocationIterator(3, 2, 5);
    const itemFrameOverrideNoTest = new BlockLocationIterator(2, 2, 10);
    const lever = new BlockLocationIterator(1, 2, 0);
    const leverOverrideTest = new BlockLocationIterator(0, 2, 13);
    test
        .startSequence()
        .thenIdle(3)
        .thenExecute(() => {
...
GameTest.register("RedstoneTests", "comparator_container", (test) => {
    const aLeft = new BlockLocationIterator(6, 2, 2);
    const aRight = new BlockLocationIterator(1, 2, 2);
    test.assertRedstonePower(aLeft, 14);
    test.assertRedstonePower(aRight, 15);
    const bLeft = new BlockLocationIterator(6, 2, 7);
    const bRight = new BlockLocationIterator(1, 2, 7);
    test.assertRedstonePower(bLeft, 0);
    test.assertRedstonePower(bRight, 15);
    const cLeft = new BlockLocationIterator(6, 2, 13);
    const cRight = new BlockLocationIterator(1, 2, 13);
...
GameTest.register("RedstoneTests", "wireredirect_nonconductor", (test) => {
    ...
    const levers = [new BlockLocationIterator(3, 2, 0), new BlockLocationIterator(1, 2, 0)];
    const fenceGatesClosed = [
        new BlockLocationIterator(2, 3, 0),
        new BlockLocationIterator(2, 3, 2),
        new BlockLocationIterator(1, 2, 2),
        new BlockLocationIterator(2, 2, 2),
        new BlockLocationIterator(3, 2, 2),
        new BlockLocationIterator(0, 2, 1),
        new BlockLocationIterator(4, 2, 1),
    ];
    const fenceGatesOpen = [new BlockLocationIterator(3, 3, 1), new BlockLocationIterator(1, 3, 1)];
    test
        .startSequence()
        .thenExecute(() => {
...
GameTest.register("RedstoneTests", "repeater_regeneration", (test) => {
    const testEx = new GameTestExtensions(test);
    const input = new BlockLocationIterator(0, 2, 0);
    const inactiveOutput = new BlockLocationIterator(6, 3, 4);
    const activeOutput = new BlockLocationIterator(6, 3, 3);
...
GameTest.register("RedstoneTests", "repeater_lock", (test) => {
    const testEx = new GameTestExtensions(test);
    const input = new BlockLocationIterator(0, 2, 2);
    const lock = new BlockLocationIterator(1, 2, 0);
    const output = new BlockLocationIterator(2, 2, 1);
    test.setBlockType(MinecraftBlockTypes.redstoneBlock, input);
    test
        .startSequence()
        .thenIdle(6)
...
GameTest.register("RedstoneTests", "wire_redirect", (test) => {
    const testEx = new GameTestExtensions(test);
    const levers = [new BlockLocationIterator(6, 2, 1), new BlockLocationIterator(3, 2, 0), new BlockLocationIterator(0, 2, 1)];
    const wires = [new BlockLocationIterator(5, 2, 4), new BlockLocationIterator(3, 2, 4), new BlockLocationIterator(1, 2, 4)];
    const fenceGates = [
        new BlockLocationIterator(5, 3, 1),
        new BlockLocationIterator(5, 3, 3),
        new BlockLocationIterator(3, 3, 1),
        new BlockLocationIterator(3, 3, 3),
        new BlockLocationIterator(1, 3, 1),
        new BlockLocationIterator(1, 3, 3),
    ];
    test
        .startSequence()
        .thenExecute(() => {
...
let observerClock = (test, initialOpenBit) => {
    const testEx = new GameTestExtensions(test);
    const outputPos = new BlockLocationIterator(2, 2, 0);
            ...
            sequence = sequence.thenWaitAfter(2, () => {
                for (let line = 0; line < 4; line++) {
                    ...
                    for (let linePos = 0; linePos < 4; linePos++) {
                        const blockWorldPos = test.worldBlockLocationIterator(linesPos[line][linePos]);
                        const block = dimension.getBlock(blockWorldPos);
                        const blockId = block.id;
                        if (linePos == expectedPos) {
                            test.assert(blockId == "minecraft:powered_repeater", "Unexpected Block State. Expected: powered. Actual: unpowered");
                        }
                        else {
                            test.assert(blockId == "minecraft:unpowered_repeater", "Unexpected Block State. Expected: unpowered. Actual: powered");
                        }
                    }
                }
            });
            ...
            sequence = sequence.thenWait(() => {
                for (let line = 0; line < 4; line++) {
                    ...
                    for (let linePos = 0; linePos < 4; linePos++) {
                        ...
                        if (linePos == expectedPos) {
                            test.assert(blockType.id == "minecraft:powered_repeater", "Unexpected Block State. Expected: powered. Actual: unpowered");
                        }
                        else {
                            test.assert(blockType.id == "minecraft:unpowered_repeater", "Unexpected Block State. Expected: unpowered. Actual: powered");
                        }
                    }
                }
            });
...
GameTest.register("RedstoneTests", "repeater_clock", (test) => {
    const testEx = new GameTestExtensions(test);
    const startPos = new BlockLocationIterator(0, 4, 0);
    const stagesPos = [
        new BlockLocationIterator(0, 1, 0),
        new BlockLocationIterator(2, 1, 0),
        new BlockLocationIterator(2, 1, 2),
        new BlockLocationIterator(0, 1, 2),
    ];
...
GameTest.register("RedstoneTests", "torch_nor", async (test) => {
    const testEx = new GameTestExtensions(test);
    const inputA = new BlockLocationIterator(4, 2, 0);
    const inputB = new BlockLocationIterator(0, 2, 0);
    const output = new BlockLocationIterator(2, 3, 0);
...
GameTest.register("RedstoneTests", "rs_latch", (test) => {
    const testEx = new GameTestExtensions(test);
    const r = new BlockLocationIterator(1, 2, 0);
    const s = new BlockLocationIterator(2, 2, 5);
    const q = new BlockLocationIterator(0, 4, 2);
    const notQ = new BlockLocationIterator(3, 4, 3);
    test
        .startSequence()
...
GameTest.register("RedstoneTests", "repeater_delay", (test) => {
    test.setBlockType(MinecraftBlockTypes.stone, new BlockLocationIterator(0, 2, 5));
    const lamp1 = new BlockLocationIterator(1, 2, 0);
    const lamp2 = new BlockLocationIterator(3, 2, 0);
    test
        .startSequence()
        .thenWait(() => {
...
GameTest.register("RedstoneTests", "dust_loop_depowering", (test) => {
    const source = new BlockLocationIterator(2, 2, 0);
    const input = new BlockLocationIterator(2, 2, 1);
    const pointA = new BlockLocationIterator(4, 2, 1);
    const pointB = new BlockLocationIterator(0, 2, 16);
    const pointC = new BlockLocationIterator(4, 2, 1);
    const pointD = new BlockLocationIterator(0, 2, 16);
...
GameTest.register("RedstoneTests", "lever_power", (test) => {
    const powered = [
        new BlockLocationIterator(1, 2, 0),
        new BlockLocationIterator(1, 2, 3),
        new BlockLocationIterator(2, 2, 1),
        new BlockLocationIterator(2, 2, 2),
        new BlockLocationIterator(0, 2, 1),
        new BlockLocationIterator(0, 2, 2),
        new BlockLocationIterator(1, 3, 1),
        new BlockLocationIterator(1, 3, 2),
        new BlockLocationIterator(1, 1, 1),
        new BlockLocationIterator(1, 1, 2),
        new BlockLocationIterator(1, 2, 2),
    ];
    const leverPos = new BlockLocationIterator(1, 2, 1);
    test.pullLever(leverPos);
    const pointA = new BlockLocationIterator(0, 1, 0);
    const pointB = new BlockLocationIterator(2, 3, 3);
    test.succeedIf(() => {
        pointA
...
GameTest.register("RedstoneTests", "dust_propagation", (test) => {
    let levels = new Map();
    const origin = new BlockLocationIterator(2, 2, 1);
    {
        origin
            .blocksBetween(new BlockLocationIterator(2, 2, 17))
            .forEach((p) => levels.set(p, Math.max(15 - distManhattan(origin, p), 0)));
    }
    {
        levels.set(new BlockLocationIterator(3, 2, 2), 13);
        levels.set(new BlockLocationIterator(3, 2, 9), 6);
        const leftRoot = new BlockLocationIterator(4, 2, 2);
        leftRoot
            .blocksBetween(new BlockLocationIterator(4, 2, 14))
            .forEach((p) => levels.set(p, Math.max(12 - distManhattan(leftRoot, p), 0)));
    }
    {
        levels.set(new BlockLocationIterator(1, 2, 3), 12);
        const rightRoot = new BlockLocationIterator(0, 2, 3);
        rightRoot
            .blocksBetween(new BlockLocationIterator(0, 2, 14))
...
GameTest.register("RedstoneTests", "torch_nand", (test) => {
    ...
    const inputA = new BlockLocationIterator(4, 2, 0);
    const inputB = new BlockLocationIterator(0, 2, 0);
    const output = new BlockLocationIterator(2, 2, 4);
    test
        .startSequence()
...
GameTest.register("RedstoneTests", "comparator_logic", (test) => {
    const mainInput = new BlockLocationIterator(3, 2, 2);
    const sideInput = new BlockLocationIterator(1, 2, 0);
    const output = new BlockLocationIterator(0, 2, 2);
...
GameTest.register("RedstoneTests", "subtractor_logic", (test) => {
    ...
    .maxTicks(TicksPerSecond * 60)
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=RedstoneTests.js.map