...
function lookAtThree(test, blocks, blockVectorOptions) {
    const player = test.spawnSimulatedPlayer(new BlockLocationIterator(2, 9, 2), "Player");
    test
        .startSequence()
        .thenExecuteAfter(10, () => {
        ...
        const relativePos = test.relativeBlockLocationIterator(block.location);
...
GameTest.register("RaycastingTests", "player_looks_under_water", (test) => {
    var blocks = [new BlockLocationIterator(1, 1, 1), new BlockLocationIterator(2, 1, 1), new BlockLocationIterator(3, 1, 1)];
...
GameTest.register("RaycastingTests", "player_looks_at_water", (test) => {
    var blocks = [new BlockLocationIterator(1, 2, 1), new BlockLocationIterator(2, 2, 1), new BlockLocationIterator(3, 2, 1)];
    const blockVectorOptions = {
...
GameTest.register("RaycastingTests", "player_looks_under_carpet", (test) => {
    var blocks = [new BlockLocationIterator(1, 2, 0), new BlockLocationIterator(2, 2, 0), new BlockLocationIterator(3, 2, 0)];
    const blockVectorOptions = {
        includePassableBlocks: false,
        includeLiquidBlocks: false,
    };
...
GameTest.register("RaycastingTests", "player_looks_at_carpet", (test) => {
    var blocks = [new BlockLocationIterator(1, 3, 0), new BlockLocationIterator(2, 3, 0), new BlockLocationIterator(3, 3, 0)];
    const blockVectorOptions = {
        includePassableBlocks: true,
        includeLiquidBlocks: false,
    };
...
GameTest.register("RaycastingTests", "get_block_from_vector", (test) => {
    ...
    const bars = dimension.getBlockFromRay(test.worldLocation(new Location(0.5, 2, 1.5)), { x: 1, y: 0, z: 0 }, blockVectorOptions);
    ...
    const banner = dimension.getBlockFromRay(test.worldLocation(new Location(0.5, 2, 1.5)), { x: 1, y: 0, z: 0 }, blockVectorOptions);
    ...
    const water = dimension.getBlockFromRay(test.worldLocation(new Location(0.5, 2, 1.5)), { x: 1, y: 0, z: 0 }, blockVectorOptions);
...
GameTest.register("RaycastingTests", "get_entity_from_vector", (test) => {
    let dimension = test.getDimension();
    test.spawnWithoutBehaviors("creeper", new BlockLocationIterator(3, 2, 1));
    ...
    const creepersReversed = dimension.getEntitiesFromRay(test.worldLocation(new Location(5.5, 2.5, 1.5)), { x: -1, y: 0, z: 0 });
    ...
    test.assertEntityInstancePresent(creepersReversed[1], new BlockLocationIterator(2, 2, 1));
    // test blocks stop the entity raycast
    const blockedCreepers = dimension.getEntitiesFromRay(test.worldLocation(new Location(5.5, 3.5, 1.5)), { x: -1, y: 0, z: 0 });
    test.assert(blockedCreepers.length == 0, "Expected the block to stop the raycast");
    test.succeed();
})
    .setupTicks(4) // time for water to convert from dynamic to static type
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=RaycastingTests.js.map