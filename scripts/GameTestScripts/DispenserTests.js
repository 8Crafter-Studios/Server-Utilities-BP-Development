...
import { BlockLocationIterator, MinecraftBlockTypes, MinecraftItemTypes, ItemStack } from "@minecraft/server";
...
GameTest.register("DispenserTests", "dispenser_shears_sheep", (test) => {
    ...
    const entityLoc = new BlockLocationIterator(1, 2, 1);
    ...
    test.pressButton(new BlockLocationIterator(0, 2, 0));
    test.assertEntityPresent(sheepId, entityLoc, true);
...
GameTest.register("DispenserTests", "dispenser_horsearmor_on_horse", (test) => {
    ...
    test.succeedWhen(() => {
        test.assertContainerEmpty(new BlockLocationIterator(0, 2, 1));
...
function dispenserMinecartTest(test, entityId) {
    const minecartPos = new BlockLocationIterator(1, 2, 1);
...
GameTest.register("DispenserTests", "dispenser_water", (test) => {
    const waterPos = new BlockLocationIterator(1, 2, 1);
    const dispenserPos = new BlockLocationIterator(0, 2, 1);
...
GameTest.register("DispenserTests", "dispenser_arrow_trap", (test) => {
    const sheepId = "minecraft:sheep<minecraft:ageable_grow_up>";
    const sheepPos = new BlockLocationIterator(4, 2, 2);
    test.spawn(sheepId, sheepPos);
    test.assertEntityPresent(sheepId, sheepPos, true);
    test.pullLever(new BlockLocationIterator(2, 3, 2));
...
GameTest.register("DispenserTests", "dispenser_charge_respawn_anchor", (test) => {
    ...
    test.pressButton(new BlockLocationIterator(0, 2, 0));
    const respawnAnchorPos = new BlockLocationIterator(1, 2, 1);
    const dispenserPos = new BlockLocationIterator(0, 2, 1);
...
GameTest.register("DispenserTests", "dispenser_fire", (test) => {
    test.pullLever(new BlockLocationIterator(2, 5, 1));
    const firePositions = [
        new BlockLocationIterator(2, 2, 1),
        new BlockLocationIterator(2, 4, 0),
        new BlockLocationIterator(4, 5, 1),
        new BlockLocationIterator(0, 5, 1),
        new BlockLocationIterator(2, 5, 3),
        new BlockLocationIterator(2, 7, 1),
    ];
    test.succeedWhen(() => {
...
GameTest.register("DispenserTests", "dispenser_fire_crash", (test) => {
    test.pullLever(new BlockLocationIterator(0, 2, 0));
    test.succeedOnTick(50);
})
    .maxTicks(threeSecondsInTicks)
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=DispenserTests.js.map