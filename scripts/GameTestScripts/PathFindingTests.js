...
import { MinecraftBlockTypes, BlockProperties, BlockLocationIterator, Location } from "@minecraft/server";
///
// Setup constants
///
        ...
        for (let mapColIndex = 0; mapColIndex < mapRow.length; mapColIndex += 2) {
            ...
            if (mapChar != " ") {
                ...
                for (let currentZ = spawnZ; currentZ >= spawnZ - 1; --currentZ) {
                    test.setBlockPermutation(blockPerm, new BlockLocationIterator(currentX, currentY, currentZ));
                }
            }
            --currentX;
        }
...
function placeBlocks(test, floor1, floor2, mid2, ceiling2) {
    const spawnPos = new BlockLocationIterator(5, 2, 4);
...
function createVerticalTestFunctionWithPlacementMap(counter, placementMap, tag) {
    ...
    GameTest.register("PathFindingTests", testName, (test) => {
        ...
        const bedPos = new BlockLocationIterator(1, 2, 4);
        ...
        const spawnPos = new BlockLocationIterator(5, 3, 4);
        // Do the test
...
GameTest.register("PathFindingTests", "bottleneck", (test) => {
    ...
    test.succeedWhen(() => {
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(5, 2, 2), true);
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(5, 2, 1), true);
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(1, 2, 2), true);
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(1, 2, 1), true);
    });
})
...
GameTest.register("PathFindingTests", "doorway", (test) => {
    const villagerEntitySpawnType = "minecraft:villager_v2<minecraft:spawn_farmer>";
    test.spawn(villagerEntitySpawnType, new BlockLocationIterator(2, 2, 6));
    test.succeedWhen(() => {
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(2, 2, 2), true);
    });
})
    .padding(TEST_PADDING) // Space out villager tests to stop them from confusing each other
    .batch("night")
...
GameTest.register("PathFindingTests", "doorway_with_stairs", (test) => {
    const villagerEntitySpawnType = "minecraft:villager_v2<minecraft:spawn_farmer>";
    test.spawn(villagerEntitySpawnType, new BlockLocationIterator(2, 2, 8));
    test.succeedWhen(() => {
        test.assertEntityPresent(villagerEntitySpawnType, new BlockLocationIterator(2, 2, 2), true);
    });
})
...
GameTest.register("PathFindingTests", "big_obstacle_course", (test) => {
    const bedPos = new BlockLocationIterator(4, 3, 6);
    const spawnPos = new BlockLocationIterator(5, 3, 4);
    const villagerEntitySpawnType = "minecraft:villager_v2<minecraft:spawn_farmer>";
...
GameTest.register("PathFindingTests", "trapdoors", (test) => {
    ...
    const targetPos = new BlockLocationIterator(3, 2, 2);
...
//# sourceMappingURL=PathFindingTests.js.map