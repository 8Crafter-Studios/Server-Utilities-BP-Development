...
let respawnanchor_explosion = (test) => {
    const player = test.spawnSimulatedPlayer(new BlockLocationIterator(2, 2, 2));
    const anchorPos = new BlockLocationIterator(1, 2, 2);
...
GameTest.register("RespawnAnchorTests", "inwater_explosion_not_destructive", (test) => respawnanchor_explosion(test)).tag(GameTest.Tags.suiteDefault);
...
GameTest.register("RespawnAnchorTests", "onland_explosion_destructive", (test) => {
    const player = test.spawnSimulatedPlayer(new BlockLocationIterator(2, 2, 1));
    const anchorPos = new BlockLocationIterator(1, 2, 1);
    const woolPos = anchorPos.above();
    test
        .startSequence()
        .thenExecute(() => {
...
//# sourceMappingURL=RespawnAnchorTests.js.map