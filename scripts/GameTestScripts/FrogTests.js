...
import { BlockLocationIterator, MinecraftItemTypes, Location, TicksPerSecond, MinecraftBlockTypes } from "@minecraft/server";
import GameTestExtensions from "./GameTestExtensions.js";
GameTest.register("FrogTests", "frog_jump", (test) => {
    const frogEntityType = "minecraft:frog";
    const startPos = new BlockLocationIterator(0, 7, 0);
    const endPos = new BlockLocationIterator(3, 7, 0);
...
GameTest.register("FrogTests", "frog_eat_slime_drop_slimeball", (test) => {
    const frogEntityType = "minecraft:frog";
    const startPos = new BlockLocationIterator(1, 2, 1);
    test.spawn(frogEntityType, startPos);
...
GameTest.register("FrogTests", "frog_lay_egg", (test) => {
    const startPosFrogOne = new BlockLocationIterator(0, 4, 1);
    const startPosFrogTwo = new BlockLocationIterator(4, 4, 1);
    const startPosPlayer = new BlockLocationIterator(2, 4, 0);
    const spawnPos = new BlockLocationIterator(2, 4, 3);
    ...
    .tag(GameTest.Tags.suiteDefault);
GameTest.register("FrogTests", "frog_egg_flow_water", (test) => {
    //This test verifies that frogs only lay egg on water that has a flat surface, and not on the "flowing" part of water
    const startPosFrogOne = new BlockLocationIterator(1, 2, 1);
    const startPosFrogTwo = new BlockLocationIterator(2, 2, 1);
    const startPosPlayer = new BlockLocationIterator(1, 3, 3);
    const flatWaterPos = new BlockLocationIterator(5, 4, 4); //This position is where the water is flat
    let playerSim = test.spawnSimulatedPlayer(startPosPlayer, "playerSim_frog");
    let frogOne = test.spawn("minecraft:frog", startPosFrogOne);
    let frogTwo = test.spawn("minecraft:frog", startPosFrogTwo);
    const testEx = new GameTestExtensions(test);
    ...
    .maxTicks(TicksPerSecond * 90)
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=FrogTests.js.map