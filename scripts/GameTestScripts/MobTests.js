...
import { BlockLocationIterator, MinecraftBlockTypes, MinecraftEffectTypes, MinecraftItemTypes } from "@minecraft/server";
import GameTestExtensions from "./GameTestExtensions.js";
const TicksPerSecond = 20;
GameTest.register("MobTests", "zombie_burn", (test) => {
    const zombieEntityType = "minecraft:zombie";
    const zombiePosition = new BlockLocationIterator(1, 2, 1);
...
GameTest.register("MobTests", "effect_durations_longer_first", (test) => {
    ...
    const villagerPos = new BlockLocationIterator(1, 2, 1);
    const buttonPos = new BlockLocationIterator(1, 4, 0);
    const strongPotion = new BlockLocationIterator(0, 4, 0);
    const weakPotion = new BlockLocationIterator(2, 4, 0);
    ...
    test
        ...
        .thenWait(() => {
        test.assertEntityState(villagerPos, villagerId, (entity) => entity.getEffect(MinecraftEffectTypes.regeneration).amplifier == 0); // Strength level I
        test.assertEntityState(villagerPos, villagerId, (entity) => entity.getEffect(MinecraftEffectTypes.regeneration).duration > TicksPerSecond * 10);
    })
        .thenSucceed();
})
...
GameTest.register("MobTests", "drowning_test", (test) => {
    ...
    test.spawn(villagerEntitySpawnType, new BlockLocationIterator(3, 2, 2));
    test.spawn(pigSpawnType, new BlockLocationIterator(3, 2, 4));
    test.succeedWhen(() => {
...
GameTest.register("MobTests", "golem_vs_pillager", (test) => {
    ...
    const ironGolemPos = new BlockLocationIterator(3, 2, 3);
    const pillagerPos = new BlockLocationIterator(3, 2, 4);
...
GameTest.register("MobTests", "silverfish_no_suffocate", (test) => {
    const silverfishPos = new BlockLocationIterator(1, 2, 1);
...
GameTest.register("MobTests", "small_mobs_keep_head_above_water", (test) => {
    const testEx = new GameTestExtensions(test);
    const swimmerPos = new BlockLocationIterator(1, 3, 1); //When the silverfish is produced at (1, 2, 1), the silverfish is stuck in the glass below and dies, so the y-axis goes up one frame
    const swimmer = test.spawn("minecraft:silverfish", swimmerPos);
    const drownerPos = new BlockLocationIterator(5, 2, 1);
...
GameTest.register("MobTests", "axolotl_bucket_capture", (test) => {
    let playerSim = test.spawnSimulatedPlayer(new BlockLocationIterator(1, 5, 0), "playerSim_axolotl");
    let target = test.spawn("minecraft:axolotl", new BlockLocationIterator(1, 5, 2));
    const testEx = new GameTestExtensions(test);
    test
        .startSequence()
...
GameTest.register("MobTests", "axolotl_attacks_squid", (test) => {
    let axlSpawn = new BlockLocationIterator(2, 3, 2);
...
GameTest.register("MobTests", "goat_wheat_breeding", (test) => {
    let playerSim = test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 0), "playerSim_goat");
    let goat_1 = test.spawn("minecraft:goat<minecraft:ageable_grow_up>", new BlockLocationIterator(2, 2, 1));
    let goat_2 = test.spawn("minecraft:goat<minecraft:ageable_grow_up>", new BlockLocationIterator(0, 2, 1));
    const testEx = new GameTestExtensions(test);
    test
        .startSequence()
...
GameTest.register("MobTests", "ravager_flowing_water_edge_fall", (test) => {
    ...
    const spawnPos = new BlockLocationIterator(2, 6, 2);
    const targetPos = new BlockLocationIterator(2, 2, 2);
    ...
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=MobTests.js.map