...
import { BlockAreaSize, BlockLocationIterator, GameMode, Location, world } from "@minecraft/server";
GameTest.register("EntityQueryTests", "world_player_query", (test) => {
    const player = test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 1), "world_player_1");
    test.spawnSimulatedPlayer(new BlockLocationIterator(0, 2, 1), "world_player_2");
    test
        .startSequence()
        .thenExecuteAfter(2, () => {
...
GameTest.register("EntityQueryTests", "dimension_player_query", async (test) => {
    const player = test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 1), "dimension_player_1");
    test.spawnSimulatedPlayer(new BlockLocationIterator(0, 2, 1), "dimension_player_2");
    await test.idle(2);
    let options = { name: player.nameTag };
...
GameTest.register("EntityQueryTests", "dimension_entity_query", (test) => {
    ...
    // e5|e4|e3
    // e2|e1|e0
    const e0 = test.spawn("minecraft:cow<minecraft:ageable_grow_up>", new BlockLocationIterator(1, 2, 1));
    const e1 = test.spawn("minecraft:cow<minecraft:ageable_grow_up>", new BlockLocationIterator(3, 2, 1));
    const e2 = test.spawn("minecraft:cow<minecraft:ageable_grow_up>", new BlockLocationIterator(5, 2, 1));
    const e3 = test.spawn("minecraft:husk<minecraft:ageable_grow_up>", new BlockLocationIterator(1, 2, 3));
    const e4 = test.spawn("minecraft:zombie<minecraft:ageable_grow_up>", new BlockLocationIterator(3, 2, 3));
    const e5 = test.spawn("minecraft:sheep<minecraft:ageable_grow_up>", new BlockLocationIterator(5, 2, 3));
    const e6 = test.spawn("minecraft:sheep<minecraft:ageable_grow_up>", new BlockLocationIterator(1, 2, 5));
    const e7 = test.spawnSimulatedPlayer(new BlockLocationIterator(3, 2, 5), p1Name);
    const e8 = test.spawnSimulatedPlayer(new BlockLocationIterator(5, 2, 5), p2Name);
    const dimension = test.getDimension();
    test
        .startSequence()
        .thenExecuteAfter(2, () => {
        ...
        e8.lookAtBlock(new BlockLocationIterator(5, 2, 6)); // Look down ~48 degrees
    })
        .thenExecuteAfter(5, () => {
...
GameTest.register("EntityQueryTests", "world_player_query_positional_option_exception", (test) => {
    let assertQueryPositionalOptionException = (options, propertyName) => {
        try {
            ...
            test.assert(ex === `EntityQueryOptions property '${propertyName}' is incompatible with function world.getPlayers`, `Unexpected exception: ${ex}`);
        }
    };
    test.spawnSimulatedPlayer(new BlockLocationIterator(1, 2, 1), "world_player_1");
    let options = {};
    ...
    .structureName("ComponentTests:platform")
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=EntityQueryTests.js.map