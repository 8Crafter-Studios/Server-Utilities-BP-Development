...
GameTest.register("FireAvoidTests", "can_walk_around_lava", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(2, 3, 4),
        targetPosition: new BlockLocationIterator(2, 3, 1),
        CanTakeDamage: false,
...
GameTest.register("FireAvoidTests", "dont_cut_corner_over_fire", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(2, 2, 2),
        CanTakeDamage: false,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "dont_cut_corner_over_fire_far", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(5, 2, 1),
        CanTakeDamage: false,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "dont_walk_into_magma", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(3, 2, 1),
        CanTakeDamage: false,
...
GameTest.register("FireAvoidTests", "fire_maze", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(8, 2, 4),
        CanTakeDamage: false,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "fire_maze_3d", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 3, 1),
        targetPosition: new BlockLocationIterator(7, 2, 11),
        CanTakeDamage: false,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "golem_chase_zombie_over_fire", (test) => {
    const zombieLocation = new BlockLocationIterator(7, 2, 1);
    const zombieType = "minecraft:zombie";
    test.spawnWithoutBehaviors(zombieType, zombieLocation);
    test.spawn("minecraft:iron_golem", new BlockLocationIterator(1, 2, 2));
...
GameTest.register("FireAvoidTests", "villager_dont_flee_over_fire", (test) => {
    test.spawnWithoutBehaviors("minecraft:zombie", new BlockLocationIterator(5, 2, 1));
    const villager = test.spawn("minecraft:villager_v2", new BlockLocationIterator(4, 2, 1));
    const startingHealth = villager.getComponent("minecraft:health").current;
    test.runAfterDelay(runWalkTestTicks - 1, () => {
...
GameTest.register("FireAvoidTests", "walk_far_out_of_magma", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(4, 2, 1),
        CanTakeDamage: true,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "walk_far_out_of_magma_diagonal", (test) => {
    runWalkTest(test, {
        spawnPosition: new BlockLocationIterator(1, 2, 1),
        targetPosition: new BlockLocationIterator(3, 2, 3),
        CanTakeDamage: true,
        shouldReachTarget: true,
    });
})
...
GameTest.register("FireAvoidTests", "zombie_chase_villager_over_fire", (test) => {
    test.spawnWithoutBehaviors("minecraft:villager_v2", new BlockLocationIterator(5, 2, 1));
    const zombie = test.spawn("minecraft:zombie", new BlockLocationIterator(1, 2, 1));
    test.succeedWhenEntityPresent("minecraft:zombie", new BlockLocationIterator(4, 2, 1), true);
})
    .maxTicks(TicksPerSecond * 10)
    .batch("night")
    .tag(GameTest.Tags.suiteDefault);
//# sourceMappingURL=FireAvoidTests.js.map