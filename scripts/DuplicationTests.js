...
import { BlockLocationIterator, MinecraftItemTypes } from "@minecraft/server";

function poweredRailTest(test, pulseTicks) {
  test.pulseRedstone(new BlockLocationIterator(1, 2, 3), pulseTicks);

  test
    .startSequence()
    .thenIdle(8)
    .thenExecute(() => test.assertItemEntityCountIs(MinecraftItemTypes.goldenRail, new BlockLocationIterator(1, 2, 1), 1.0, 1)) // powered rail
    .thenSucceed();
}
...
GameTest.register("DuplicationTests", "detector_rail", (test) => {
  test.spawn("minecraft:minecart", new BlockLocationIterator(1, 3, 2));

  test
    .startSequence()
    .thenIdle(8)
    .thenExecute(() =>
      test.assertItemEntityCountIs(MinecraftItemTypes.detectorRail, new BlockLocationIterator(1, 2, 1), 1.0, 1)
    )
    .thenSucceed();
}).tag(GameTest.Tags.suiteDefault);

function railClassicTest(test, pulseTicks) {
  test.pulseRedstone(new BlockLocationIterator(1, 5, 5), pulseTicks);

  test
    .startSequence()
    .thenIdle(3)
    .thenExecute(() => test.assertItemEntityCountIs(MinecraftItemTypes.rail, new BlockLocationIterator(1, 4, 2), 1.0, 0))
    .thenSucceed();
}
