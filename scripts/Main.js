...
import { Block, BlockPermutation, Entity, EntityInventoryComponent, EntityScaleComponent, ItemDurabilityComponent, ItemLockMode, ItemStack, Player, ScriptEventSource, WeatherType, world, BlockInventoryComponent /*, EntityEquipmentInventoryComponent*/, EquipmentSlot, EntityEquippableComponent, BlockTypes, MolangVariableMap, EnchantmentTypes, EntityBreathableComponent, EntityColorComponent, EntityFlyingSpeedComponent, EntityFrictionModifierComponent, EntityGroundOffsetComponent, EntityHealthComponent, EntityMarkVariantComponent, EntityPushThroughComponent, EntitySkinIdComponent, EntityTameableComponent, SignSide, ItemEnchantableComponent, DyeColor, EntityDamageCause, } from "@minecraft/server";
...
export function mainEval(x) {
    return eval(x);
}
export function indirectMainEval(x) {
    return eval?.(x);
}
export function mainRun(x, ...args) {
    return x(...args);
}
try {
...
subscribedEvents.afterEffectAdd = world.afterEvents.effectAdd.subscribe((event) => {
    ...
    try {
        getPlayersWithAnyOfTags([
            ...
            let pn = new PlayerNotifications(p);
            srun(() => p.playSound(pn.getEffectAddNotificationsNotificationSound.soundId, {
...
subscribedEvents.afterEntityLoad = world.afterEvents.entityLoad.subscribe((event) => {
    ...
    try {
        getPlayersWithAnyOfTags([
            ...
            let pn = new PlayerNotifications(p);
            srun(() => p.playSound(pn.getEntityLoadNotificationsNotificationSound.soundId, {
                pitch: pn
...
subscribedEvents.afterEntitySpawn = world.afterEvents.entitySpawn.subscribe((event) => {
    ...
    try {
        getPlayersWithAnyOfTags([
            ...
            ? v.hasTag("getNotifiedOfPlayerInventorySaveStorageEntitySpawns")
            : true)
            .forEach((p) => {
            psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§eentitySpawn§r] Entity of type ${event.entity.typeId} with the id ${event.entity.id} was spawned in ${dimensionTypeDisplayFormatting[event.entity?.dimension?.id]} at ${JSON.stringify(event.entity?.location)} with the cause "${event.cause}". `);
            let pn = new PlayerNotifications(p);
            srun(() => p.playSound(pn.getEntitySpawnNotificationsNotificationSound
                .soundId, {
                pitch: pn
    ...
    world.afterEvents.gameRuleChange.subscribe((event) => {
        ...
        try {
            getPlayersWithTags("getGameRuleChangeNotifications")
                .filter((p) => !p.hasTag("excludeGameRuleChangeNotificationsFor:" +
                event.rule))
                .forEach((p) => {
                psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§egameRuleChange§r] "${event.rule}" was changed to ${event.value}. `);
                let pn = new PlayerNotifications(p);
                srun(() => p.playSound(pn.getGameRuleChangeNotificationsNotificationSound
                    .soundId, {
                    pitch: pn
...
subscribedEvents.afterPlayerGameModeChange =
    world.afterEvents.playerGameModeChange.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("playerGameModeChangeAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
        try {
            getPlayersWithTags("getGameModeChangeNotifications")
                .filter((p) => !p.hasTag("excludeGameModeChangeNotificationsFor:" +
                event.player.name) &&
                !p.hasTag("excludeGameModeChangeNotificationsFrom:" +
                    event.fromGameMode) &&
                !p.hasTag("excludeGameModeChangeNotificationsTo:" +
                    event.toGameMode))
                .forEach((p) => {
                psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§eplayerGameModeChange§r][${event.player.name}] Changed from ${event.fromGameMode} to ${event.toGameMode}. `);
                let pn = new PlayerNotifications(p);
                srun(() => p.playSound(pn
...
subscribedEvents.afterWeatherChange = world.afterEvents.weatherChange.subscribe((event) => {
    ...
    try {
        getPlayersWithTags("getWeatherChangeNotifications")
            .filter((p) => !p.hasTag("excludeWeatherChangeNotificationsTo:" +
            event.newWeather) &&
            ...
            .forEach((p) => {
            psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§eweatherChange§r] Weather in ${event.dimension} changed from ${event.previousWeather} to ${event.newWeather}. `);
            let pn = new PlayerNotifications(p);
            srun(() => p.playSound(pn.getWeatherChangeNotificationsNotificationSound
                .soundId, {
                pitch: pn
    ...
    if (event.itemStack?.typeId === "andexdb:selection_tool") {
        try {
            ...
            if (!!!loc) {
                ...
                srun(() => {
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2"
                        : "andexdb:xz_axis_particle_pos1", Vector.add(loc, { x: 0.5, y: 1.005, z: 0.5 }));
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2_north"
                        : "andexdb:xz_axis_particle_pos1_north", Vector.add(loc, { x: 0.5, y: 0.5, z: 1.005 }));
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2_east"
                        : "andexdb:xz_axis_particle_pos1_east", Vector.add(loc, { x: -0.005, y: 0.5, z: 0.5 }));
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2_down"
                        : "andexdb:xz_axis_particle_pos1_down", Vector.add(loc, { x: 0.5, y: -0.005, z: 0.5 }));
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2_south"
                        : "andexdb:xz_axis_particle_pos1_south", Vector.add(loc, { x: 0.5, y: 0.5, z: -0.005 }));
                    event.source.spawnParticle(mode
                        ? "andexdb:xz_axis_particle_pos2_west"
                        : "andexdb:xz_axis_particle_pos1_west", Vector.add(loc, { x: 1.005, y: 0.5, z: 0.5 }));
                });
            }
        }
        catch (e) {
...
subscribedEvents.afterItemStartUseOn =
    world.afterEvents.itemStartUseOn.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("itemStartUseOnAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
...
subscribedEvents.afterLeverAction = world.afterEvents.leverAction.subscribe((event) => {
    ...
    try {
        getPlayersWithTags("getLeverActionNotifications")
            .filter((p) => !p.hasTag("excludeLeverActionNotificationsTo:" +
            event.isPowered) &&
            !p.hasTag("excludeLeverActionNotificationsIn:" +
                event.dimension) &&
            !p.hasTag("excludeLeverActionNotificationsBy:" +
                event.player.name) &&
            !p.hasTag("excludeLeverActionNotificationsAt:" +
                Object.values(event.block.location).join(",")))
            .forEach((p) => {
            psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§eleverAction§r][${event.player.name}] Lever in ${dimensionTypeDisplayFormatting[event.dimension.id]} at ${vTStr(event.block.location)} turned ${event.isPowered ? "ON" : "OFF"}. `);
            let pn = new PlayerNotifications(p);
            srun(() => p.playSound(pn.getLeverActionNotificationsNotificationSound
                .soundId, {
                pitch: pn
    ...
    world.afterEvents.playerBreakBlock.subscribe((event) => {
        if (!!event?.itemStackBeforeBreak?.getDynamicProperty("afterPlayerBreakBlockCode")) {
            try {
                ...
                console.error(e, e.stack);
                world.getAllPlayers().forEach((currentplayer) => {
                    if (currentplayer.hasTag("itemAfterPlayerBreakBlockCodeDebugErrors")) {
                        currentplayer.sendMessage(e + e.stack);
                    }
                });
            }
        ...
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("playerBreakBlockAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
...
subscribedEvents.afterPlayerInteractWithBlock =
    world.afterEvents.playerInteractWithBlock.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("playerInteractWithBlockAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
        try {
            if ((["minecraft:respawn_anchor", "minecraft:tnt"].includes(event.block.typeId) &&
                event.block.dimension.id == "minecraft:overworld") ||
                (["minecraft:bed", "minecraft:tnt"].includes(event.block.typeId) &&
                ...
                ([
                    ...
                    event.block.dimension.id == "minecraft:overworld")) {
                getPlayersWithTags("getBlockInteractTriggerExplosionNotifications")
                    .filter((p) => !p.hasTag("excludeBlockInteractTriggerExplosionNotificationsIn:" +
                    event.block.dimension) &&
                    (!!event.player
                        ? !p.hasTag("excludeBlockInteractTriggerExplosionNotificationsBy:" +
                            event.player?.name) &&
                            !p.hasTag("excludeBlockInteractTriggerExplosionNotificationsById:" +
                                event.player.id)
                        : !p.hasTag("excludeBlockInteractTriggerExplosionNotificationsWithNoSource")) &&
                    !p.hasTag("excludeBlockInteractTriggerExplosionNotificationsBlockType:" +
                        event.block.typeId))
                    .forEach((p) => {
                    psend(p, `§r§f[§l§dServer§r§f]${world.getDynamicProperty("serverNotificationSpacer") ?? ""}[§eexplosiveBlockInteraction§r] ${!!event.player
                        ? `${event.player.name ??
...
subscribedEvents.afterPlayerJoin = world.afterEvents.playerJoin.subscribe((event) => {
    ...
    if (!!(ban
        ...
        try {
            ...
            let b = ban
                ?.getValidBans()
                ?.idBans?.sort((a, b) => 1 - 2 * Number(a?.banDate > b?.banDate))
                ?.find((_) => _?.playerId == event?.playerId) ??
                ban
                    .getValidBans()
                    .nameBans?.sort((a, b) => 1 - 2 * Number(a?.banDate > b?.banDate))
...
subscribedEvents.afterPressurePlatePop =
    world.afterEvents.pressurePlatePop.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("pressurePlatePopAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
...
subscribedEvents.afterPressurePlatePush =
    world.afterEvents.pressurePlatePush.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("pressurePlatePushAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
...
subscribedEvents.afterProjectileHitEntity =
    world.afterEvents.projectileHitEntity.subscribe((event) => {
        try {
            ...
            console.error(e, e.stack);
            world.getAllPlayers().forEach((currentplayer) => {
                if (currentplayer.hasTag("projectileHitEntityAfterEventDebugErrors")) {
                    currentplayer.sendMessage(e + e.stack);
                }
            });
        }
...
§l§eminecraft:inventory: §r§9{§eSlots Filled: §r§c${block.getComponent("inventory")
                                    ...
                                    ? `
§l§eminecraft:piston: §r§9{§eIs Moving: §r${((b) => (b ? "§2" : "§4") + String(b))(block.getComponent("piston")
                                        .isMoving)}§a, §eState: §r§u${block.getComponent("piston").state}§a, §eAttatched Block Count: §r§c${block
                                        .getComponent("piston")
                                        .getAttachedBlocks().length}§9}`
                                    : ""}${!!block.getComponent("record_player")
                                    ? `
§l§eminecraft:recordPlayer: §r§9{§eIs Playing: §r${((b) => (b ? "§2" : "§4") + String(b))(block
                                        .getComponent("record_player")
                                        .isPlaying())}§9}`
                                    : ""}${!!block.getComponent("sign")
                                    ? `
§l§eminecraft:sign: §r§9{§eIs Waxed: §r${((b) => (b ? "§2" : "§4") + String(b))(block.getComponent("sign").isWaxed)}§a, §eF Dye: §r§u${block
                                        .getComponent("sign")
                                        .getTextDyeColor(SignSide.Front) ?? "null"}§a, §eB Dye: §r§u${block
                                        .getComponent("sign")
                                        .getTextDyeColor(SignSide.Back) ?? "null"}§a, §eF Text Length: §r§c${block
                                        .getComponent("sign")
                                        .getText(SignSide.Front)
                                        .length}§a, §eB Text Length: §r§c${block
                                        .getComponent("sign")
                                        .getText(SignSide.Back).length}§a, §eF Is Raw Text: §r${((b) => (b ? "§2" : "§4") + String(b))(!!tryget(() => block
                                        .getComponent("sign")
                                        .getRawText(SignSide.Front)))}§a, §eB Is Raw Text: §r${((b) => (b ? "§2" : "§4") + String(b))(!!tryget(() => block
                                        .getComponent("sign")
                                        .getRawText(SignSide.Back)))}§9}`
                        ...
                        if (config.chatRanks.showRanksOnPlayerNameTags) {
                            if (!playerList2[index].hasTag("doNotSetNameTag")) {
                                ...
                                let rank = playerList2[indexb]
                                    ...
                                    .join(String(playerList2[indexb].getDynamicProperty("andexdbPersonalSettings:chatNameAndMessageSeparator") ??
                                    world.getDynamicProperty("andexdbSettings:chatNameAndMessageSeparator") ??
                                    " "));
                                if (rank == "") {
                                    let tags = playerList2[indexb].getTags();
                                ...
                                playerList2[indexb].nameTag =
                                    (showDimension
                                        ...
                                        (showHealth
                                            ? "§r§f[" +
                                                playerList2[indexb].getComponent("health").currentValue +
                                                "/" +
                                                playerList2[indexb].getComponent("health").effectiveMax +
                                                "] "
                                            : ""); /*(
...
subscribedEvents.afterScriptEventReceive =
    system.afterEvents.scriptEventReceive.subscribe((event) => {
        ...
        if (id == "andexdb:entityScaleInitSignal") {
            world
                .getDimension("overworld")
                .runCommand(`/scriptevent andexsa:entityScaleInitSignalReceivedByDebugSticks ${format_version}`);
            if (entity_scale_format_version != null &&
            ...
            world
                .getDimension("overworld")
                .runCommand(`/scriptevent andexsa:entityScaleTestSignalReceivedByDebugSticks ${format_version}`);
            if (entity_scale_format_version != null &&
                message.trim() != entity_scale_format_version) {
                globalThis.multipleEntityScaleVersionsDetected = true;
            }
        ...
        if (id == "andexdb:cmd") {
            chatCommands({
                ...
                newMessage: message.replaceAll("\\@\\", "@"),
                player: new executeCommandPlayerW(new WorldPosition(tryget(() => (initiator ??
                    sourceEntity ??
                    sourceBlock).location) ?? { x: 0, y: 0, z: 0 }, tryget(() => (initiator ?? sourceEntity).getRotation()) ?? { x: 0, y: 0 }, tryget(() => (initiator ??
                    sourceEntity ??
                    sourceBlock).dimension) ?? overworld, (initiator ?? sourceEntity), sourceBlock)),
            });
        }
            ...
            let effectsList = [players[playerTargetB].getComponents[0]]*/
                    ...
                    try {
                        entityViewedEntityType =
                            players[playerTargetB].getEntitiesFromViewDirection()[0].entity
                                .typeId;
                    }
                    catch (e) {
                    ...
                    try {
                        entityViewedEntityDistance =
                            players[playerTargetB].getEntitiesFromViewDirection()[0].distance;
                    }
                    catch (e) {
                ...
                console.warn(index);*/
                        if (Number(index) != 0) {
                            componentList = String([
                                ...
                                players[playerTargetB].getComponents()[index].typeId,
                            ]).split(",");
                        } /*
                console.warn(targetList);*/
                    }
        ...
        if (id == "andexdb:entityDebug") {
            ...
            if (message.startsWith("players:") &&
                ...
                form2
                    ...
                    if (selectionType == 0) {
                        playerTargetB =
                            players[playerViewerB].getEntitiesFromViewDirection()[0].entity;
                    }
                    if (selectionType == 1) {
                        playerTargetB = world
                            .getDimension("overworld")
    ...
    form.button("Entity Debug§b", "textures/ui/debug_glyph_color");*/ /*
        ...
        if (id == "andexdb:itemLoreInventoryModifier" ||
    ...
    form.textField("Item Data", "Trigger Event", "");*/
                ...
                form.dropdown("From Container Type", [
                    "Player",
                    "§4Facing Entity",
                ...
                form.textField("From Container Block", "overworld, 500, 60, 500", players[playerTargetB].dimension.id +
                    ", " +
                    players[playerTargetB].location.x +
                    ", " +
                    players[playerTargetB].location.y +
                    ", " +
                    players[playerTargetB].location.z);
                form.dropdown("To Container Type", [
                    "Player",
                    "§4Facing Entity",
                    "§4Entity At Block Location",
                    "§4Facing Block",
                ...
                form.textField("To Container Block", "overworld, 500, 60, 500", players[playerTargetB].dimension.id +
                    ", " +
                    players[playerTargetB].location.x +
                    ", " +
                    players[playerTargetB].location.y +
                ...
                form.textField("Container Block", "overworld, 500, 60, 500", players[playerTargetB].dimension.id +
                    ", " +
                    players[playerTargetB].location.x +
                    ", " +
                    players[playerTargetB].location.y +
                    ", " +
                    players[playerTargetB].location.z);
                form.dropdown("Other Container Type", [
                    "Player",
                    "§4Facing Entity",
                    "§4Entity At Block Location",
                    "§4Facing Block",
                ...
                form.textField("Other Container Block", "overworld, 500, 60, 500", players[playerTargetB].dimension.id +
                    ", " +
                    players[playerTargetB].location.x +
                    ", " +
                    players[playerTargetB].location.y +
    ...
    if (Number(slotType) == 1) { try{let a = players[playerTargetB].getComponent("equipment_inventory") as EntityEquipmentInventoryComponent; item = a.getEquipmentSlot(equipmentPlayerSlotsList[Number(slotNumber)])} catch(e){if (Boolean(debug2) == true) { console.error(e, e.stack); }};};*/
                    let transferFromContainerBlockB = world
                        .getDimension(String(transferFromContainerBlock).split(", ")[0])
                        .getBlock({
                    ...
                    let transferToContainerBlockB = world
                        .getDimension(String(transferToContainerBlock).split(", ")[0])
                        .getBlock({
                    ...
                    let moveFromContainerBlockB = world
                        .getDimension(String(moveFromContainerBlock).split(", ")[0])
                        .getBlock({
                    ...
                    let moveToContainerBlockB = world
                        .getDimension(String(moveToContainerBlock).split(", ")[0])
                        .getBlock({
                    ...
                    let swapContainerBlockB = world
                        .getDimension(String(swapContainerBlock).split(", ")[0])
                        .getBlock({
                    ...
                    let swapOtherContainerBlockB = world
                        .getDimension(String(swapOtherContainerBlock).split(", ")[0])
                        .getBlock({
    ...
    try{ durability2.damage = Number(10); } catch(e){if (Boolean(debug2) == true) { console.error(e, e.stack)}; }*/
        ...
        moveFromSlotB = undefined*/
                        ...
                        switch (moveFromContainerType) {
                            case 4:
                                moveFromContainerB =
                                    moveFromContainerBlockB.getComponent("inventory");
                                break;
                        }
                        ...
                        switch (moveToContainerType) {
                            case 4:
                                moveToContainerB =
                                    moveToContainerBlockB.getComponent("inventory");
                                break;
                        }
                        try {
        ...
        moveFromSlotB = undefined*/
                        ...
                        if (Number(swapSlot) > 35 &&
                            ...
                            try {
                                ...
                                itemA = swapContainerB
                                    .getEquipment(equipmentPlayerSlotsList[Number(swapSlot)])
                                    .clone();
                            }
                            catch (e) {
                                if (Boolean(debug2) == true) {
                        ...
                        switch (swapContainerType) {
                            case 4:
                                swapContainerB =
                                    swapContainerBlockB.getComponent("inventory");
                                break;
                        }
                        ...
                        if (Number(swapOtherSlot) > 35) {
                            try {
                                ...
                                itemB = swapOtherContainerB
                                    .getEquipment(equipmentPlayerSlotsList[Number(swapOtherSlot)])
                                    .clone();
                            }
                            catch (e) {
                                if (Boolean(debug2) == true) {
                        ...
                        switch (swapOtherContainerType) {
                            case 4:
                                swapOtherContainerB =
                                    swapOtherContainerBlockB.getComponent("inventory");
                                break;
                        }
                        try {
                            if (itemB == undefined) {
                        ...
                        switch (transferFromContainerType) {
                            case 4:
                                transferFromContainerB =
                                    transferFromContainerBlockB.getComponent("inventory");
                                break;
                        }
                        ...
                        switch (transferToContainerType) {
                            case 4:
                                transferToContainerB =
                                    transferToContainerBlockB.getComponent("inventory");
                                break;
                        }
                        try {
        ...
        if (id == "andexdb:inventoryTransfer") {
            ...
            for (const index in players) {
                ...
                targetList = String([String(targetList), players[index].nameTag]).split(",");
                                    ...
                                    form3.dropdown("Block", [
                                        "Position",
                                        "Preset 1",
                                        "Preset 2",
                                        "Preset 3",
                                    ], 0);
                                    form3.textField("From Block", "dimension, x, y, z", callerPlayer.dimension.id +
                                        ", " +
                                    ...
                                    form3.textField("To Block", "dimension, x, y, z", callerPlayer.dimension.id +
                                        ", " +
                                        Math.floor(callerPlayer.location.x) +
                                        ", " +
                                        Math.floor(callerPlayer.location.y) +
                                    ...
                                    form3
                                        ...
                                        let fromBlockPositionB = world
                                            .getDimension(String(fromBlockPosition).split(", ")[0])
                                            .getBlock({
                                        ...
                                        let toBlockPositionB = world
                                            .getDimension(String(toBlockPosition).split(", ")[0])
                                            .getBlock({
                                        ...
                                        if (Number(fromBlockSelectionMode) > 0) {
                                            ...
                                            try {
                                                fromPresetValues =
                                                    callerPlayer.getDynamicProperty("blockTransferPreset" +
                                                        String(Number(fromBlockSelectionMode) -
                                                            1));
                                            }
                                            catch (e) {
                                                console.error(e, e.stack);
                                        ...
                                        if (Number(toBlockSelectionMode) > 0) {
                                            ...
                                            try {
                                                toPresetValues =
                                                    callerPlayer.getDynamicProperty("blockTransferPreset" +
                                                        String(Number(toBlockSelectionMode) -
                                                            1));
                                            }
                                            catch (e) {
                                                console.error(e, e.stack);
                                    ...
                                    form2
                                        ...
                                        switch (transferType) {
                                            case 0:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        fromInventory.container.swapItems(Number(index +
                                                            9), Number(index +
                                                            9), toInventory.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                                    }
                                            ...
                                            case 1:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        fromInventory.container.transferItem(Number(index +
                                                            9), toInventory.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                            ...
                                            case 2:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        fromInventory.container.moveItem(Number(index +
                                                            9), Number(index +
                                                            9), toInventory.container);
                                                    }
                                                    catch (e) {
                    ...
                    switch (response) {
                        ...
                        case 1:
                            ...
                            break;*/:
                                    ...
                                    form4.dropdown("Block", [
                                        "Position",
                                        "Preset 1",
                                        "Preset 2",
                                        "Preset 3",
                                    ], 0);
                                    form4.textField("Block", "dimension, x, y, z", callerPlayer.dimension.id +
                                        ", " +
                                        Math.floor(callerPlayer.location.x) +
                                        ", " +
                                        Math.floor(callerPlayer.location.y) +
                                    ...
                                    form4
                                        ...
                                        if (Number(blockSelectionMode) > 0) {
                                            ...
                                            try {
                                                presetValues =
                                                    callerPlayer.getDynamicProperty("blockTransferPreset" +
                                                        String(Number(blockSelectionMode) -
                                                            1));
                                            }
                                            catch (e) {
                                                console.error(e, e.stack);
                                        ...
                                        switch (transferType) {
                                            case 0:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        blockPositionC.container.swapItems(Number(index), Number(index +
                                                            9), toInventory.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                            ...
                                            case 1:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        toInventory.container.transferItem(Number(index +
                                                            9), blockPositionC.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                                    }
                                            ...
                                            case 3:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        toInventory.container.moveItem(Number(index +
                                                            9), Number(index), blockPositionC.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                                    }
                                            ...
                                            case 4:
                                                for (let index = 0; index < 27; index++) {
                                                    try {
                                                        blockPositionC.container.moveItem(Number(index), Number(index +
                                                            9), toInventory.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
            ...
            for (const index in players) {
                ...
                targetList = String([String(targetList), players[index].nameTag]).split(",");
                                    ...
                                    form3.textField("To Player", "dimension, x, y, z", callerPlayer.dimension.id +
                                        ", " +
                                        Math.floor(callerPlayer.location.x) +
                                        ", " +
                                        Math.floor(callerPlayer.location.y) +
                                    ...
                                    form3
                                        ...
                                        switch (transferType) {
                                            case 0:
                                                for (let index = 0; index < 9; index++) {
                                                    try {
                                                        fromBlockPositionC.container.swapItems(Number(index +
                                                            Number(inventoryRow) *
                                                                9), Number(index +
                                                            Number(inventoryRow) *
                                                                9), toBlockPositionC.container);
                                                    }
                                                    catch (e) {
                                                        console.error(e, e.stack);
                                                    }
                                            ...
                                            case 1:
                                                for (let index = 0; index < 9; index++) {
                                                    try {
                                                        fromBlockPositionC.container.transferItem(Number(index +
                                                            Number(inventoryRow) *
                                                                9), toBlockPositionC.container);
                                                    }
                                                    catch (e) {
                                            ...
                                            case 2:
                                                for (let index = 0; index < 9; index++) {
                                                    try {
                                                        fromBlockPositionC.container.moveItem(Number(index +
                                                            Number(inventoryRow) *
                                                                9), Number(index +
                                                            Number(inventoryRow) *
                                                                9), toBlockPositionC.container);
                    ...
                    case 4:
                        ...
                        form6.textField("From Block", "dimension, x, y, z", callerPlayer.dimension.id +
                            ", " +
                            Math.floor(callerPlayer.location.x) +
                            ", " +
                            Math.floor(callerPlayer.location.y) +
                        ...
                        form6
                            ...
                            if (presetValues == undefined) {
                                form3.textField("Block Location", "dimension, x, y, z", callerPlayer.dimension.id +
                                    ", " +
                                    Math.floor(callerPlayer.location.x) +
                                    ", " +
                                    Math.floor(callerPlayer.location.y) +
                            ...
                            form3
                                ...
                                callerPlayer.setDynamicProperty("blockTransferPreset" +
                                    String(Number(presetName)), String(newBlockPresetValues));
                            })
                                .catch((e) => {
                                console.error(e, e.stack);
                            });
    ...
    console.warn([String(targetList)]);*/
        ...
        if (id == "andexdb:editEntityDynamicProperty") {
            ...
            forceShow(form, sourceEntity)
                ...
                form2.textField("New Value", "New Value", String(targetSelectorB(String(ts), "", Number(world.getAllPlayers()[0].id) ??
                    Number(world
                        .getDimension("overworld")
                        .getEntities()[0].id) ??
                    Number(world
                    ...
                    Number(world
                        .getDimension("the_end")
                        .getEntities()[0].id)).getDynamicProperty(String(dpi))));
                forceShow(form2, sourceEntity)
                    ...
                    String(targetSelectorB(String(ts), "", Number(world.getAllPlayers()[0].id) ??
                        ...
                        Number(world
                            .getDimension("the_end")
                            .getEntities()[0].id)).setDynamicProperty(String(dpi), String(newValue)));
                })
                    .catch((e) => {
                    console.error(e, e.stack);
                });
    ...
    clearVelocity*/, debug, fluidContainerColor, fluidContainerFillLevel, potionType, signFrontRawTextEnabled, signFrontRawText, signBackRawTextEnabled, signBackRawText, signFrontTextEnabled, signFrontText, signBackTextEnabled, signBackText, signFrontTextColorEnabled, signFrontTextColor, signBackTextColorEnabled, signBackTextColor, setSignIsWaxed,] = r.formValues;
                ...
                for (let index in blockPropertyValueArray) {
                    if (String(blockPropertyValueArray[index]).startsWith('"') &&
                        ...
                        blockPropertyValueArray[index] = String(blockPropertyValueArray[index]).slice(1, String(blockPropertyValueArray[index]).length -
                            1);
                    }
                    else {
    ...
    GameTest.register("StarterTests", "simpleMobTest", (test: GameTest.Test) => {
        ...
        form2.dropdown("Player Viewer", String(targetList).split(","), 0)
        form2.toggle("Debug2", false);*/
            forceShow(form2, event.sourceEntity)
                .then((to) => {
                let t = to;
                if (t.canceled)
                    return;
        ...
        if (id == "andexdb:customFormUIEditor") {
            ...
            forceShow(form.form, event.sourceEntity)
                ...
                elementValues.forEach((v, i) => {
                    switch (i % 5) {
                        case 0:
                            world.setDynamicProperty(`customUIElement:${formId}|${form.indexList[Math.floor(i / 5)]}`, `${customElementTypeIds[Number(elementValues[i])]}|${elementValues
                                .slice(i + 1, i + 4)
                                .join("|")}`);
                            break;
                        case 4:
    ...
    GameTest.register("StarterTests", "simpleMobTest", (test: GameTest.Test) => {
            ...
            if (message.startsWith("players:") &&
                ...
                form2
                    ...
                    if (Number(selectionType) == 1) {
                        ...
                        try {
                            entity2 =
                                playerList[Number(playerViewer)].getEntitiesFromViewDirection();
                            entity3 =
                                playerList[Number(playerViewer)].getEntitiesFromViewDirection()[0].entity;
                        }
                        catch (e) { }
                    }
                    let playerTargetB;
      ...
      form.textField("Trigger Event", "Trigger Event")*/ /*
        ...
        console.warn([String(targetList)]);*/
            ...
            form.textField("Entity Identifier", "Entity Identifier", 
            /*String(*/ entity3 /*)*/);
                ...
                if (Boolean(setWeather) == true && weatherDuration == "") {
                    try {
                        world
                            ...
                            .setWeather(weatherList3[weatherList2.indexOf(String(weatherType))]);
                    }
                    catch (e) {
                        console.error(e, e.stack);
                    }
            ...
            if (Boolean(removeDynamicProperty) == true) {
                ...
                if (Boolean(spawnEntity) == true) {
                    try {
                        world
                            .getDimension(dimensionList2[Number(entityDimension)])
                ...
                for (let i in playerName.slice(1)) {
                    for (let l in targets) {
                        switch (playerName.slice(1)[i].split(":")[0]) {
                            case "triggerEvent":
                                try {
                                    targets[l].triggerEvent(playerName
                                        .slice(1)[i].split(":")
                                        .slice(1)
                                        .join(":")
                                        .replaceAll("\\c", ":"));
                            ...
                            case "setProperty":
                                try {
                                    targets[l].setProperty(playerName
                                        .slice(1)[i].split(":")[1]
                                        .replaceAll("\\c", ":"), playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")
                                        .replaceAll("\\c", ":"));
                            ...
                            case "setPropertyInt":
                                try {
                                    targets[l].setProperty(playerName
                                        .slice(1)[i].split(":")[1]
                                        .replaceAll("\\c", ":"), Number(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")
                                        .replaceAll("\\c", ":")));
                            ...
                            case "setPropertyBool":
                                try {
                                    targets[l].setProperty(playerName
                                        .slice(1)[i].split(":")[1]
                                        .replaceAll("\\c", ":"), Boolean(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")
                                        .replaceAll("\\c", ":")));
        ...
        break; */
                            case "setDynamicProperty":
                                try {
                                    targets[l].setDynamicProperty(playerName
                                        .slice(1)[i].split(":")[1]
                                        .replaceAll("\\c", ":"), playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                    ...
                    for (let l in targets) {
                        try {
                            sourceEntity.sendMessage("" +
                                targets[l].id +
                                ": " +
                                targets[l].getDynamicProperty(playerName
                                    .slice(1)[i].replaceAll("\\c", ":")));
                        }
                        catch (e) {
                            console.error(e, e.stack);
                        }
        ...
        if (id == "andexdb:getWorldDynamicProperty") {
            let dynamicProperty = message.split("|");
            try {
                sourceEntity.sendMessage("a" +
                    String(world.getDynamicProperty(String(dynamicProperty[0]))));
            }
            catch (e) {
                console.error(e, e.stack);
            }
            ...
            console.warn(targetList);*/
                ...
                for (let i in playerName.slice(1)) {
                    switch (playerName[i].split(":")[0]) {
                        case "nameTag":
                            try {
                                playerList[targetList.indexOf(String(playerName[0]))].nameTag = playerName[i]
                                    .split(":")
                                    .slice(1)
                                    .join(":");
                            }
                ...
                for (let i in playerName.slice(1)) {
                    switch (playerName[i].split(":")[0]) {
                        ...
                        case "setProperty":
                            ...
                            break;
                        case "clearVelocity":
                            if (playerName[i].split(":")[1]?.toLowerCase() ==
                                "true")
                                try {
                                    playerList[targetList.indexOf(String(playerName[0]))].triggerEvent(playerName[i]
                                        .split(":")
                                        .slice(1)
                                        .join(":"));
                                }
                ...
                for (let i in playerName.slice(1)) {
                    for (let l in targets) {
                        switch (playerName.slice(1)[i].split(":")[0]) {
                            ...
                            case "setProperty":
                                try {
                                    targets[l].setProperty(playerName.slice(1)[i].split(":")[1], playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":"));
                                }
                            ...
                            case "setPropertyInt":
                                try {
                                    targets[l].setProperty(playerName.slice(1)[i].split(":")[1], Number(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")));
                                }
                            ...
                            case "setPropertyBool":
                                try {
                                    targets[l].setProperty(playerName.slice(1)[i].split(":")[1], Boolean(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")));
                                }
        ...
        break; */
                            case "setDynamicProperty":
                                try {
                                    targets[l].setDynamicProperty(playerName.slice(1)[i].split(":")[1], playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":"));
                                }
                            ...
                            case "setDynamicPropertyInt":
                                try {
                                    targets[l].setDynamicProperty(playerName.slice(1)[i].split(":")[1], Number(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")));
                                }
                            ...
                            case "setDynamicPropertyBool":
                                try {
                                    targets[l].setDynamicProperty(playerName.slice(1)[i].split(":")[1], Boolean(playerName
                                        .slice(1)[i].split(":")
                                        .slice(2)
                                        .join(":")));
                                }
            ...
            if(playerName.slice(1)[i].split(":")[1].toLowerCase() == "true"){*/:
                            ...
                            case "isSneaking":
                                try {
                                    targets[l].isSneaking = Boolean(playerName
                                        .slice(1)[i].split(":")[1]
                                        ?.toLowerCase());
                                }
                                catch (e) {
                            ...
                            case "addEffect":
                                try {
                                    targets[l].addEffect(playerName.slice(1)[i].split(":")[1], Number(playerName.slice(1)[i].split(":")[2]), {
                                        amplifier: Number(playerName
                                            .slice(1)[i].split(":")[3]),
                                        showParticles: Boolean(playerName
                                            .slice(1)[i].split(":")[4]
                                            ?.toLowerCase()),
                                    });
                                }
                            ...
                            case "teleport":
                                try {
                                    targets[l].teleport({
                                        ...
                                        checkForBlocks: Boolean(playerName
                                            .slice(1)[i].split(":")[7]),
                                        rotation: {
                                            x: Number(playerName
                                                .slice(1)[i].split(":")[5]),
                                        ...
                                        dimension: world.getDimension(playerName
                                            .slice(1)[i].split(":")[4]),
                                        keepVelocity: Boolean(playerName
                                            .slice(1)[i].split(":")[8]),
                                    });
                                }
                                catch (e) {
                            ...
                            case "extinguishFire":
                                try {
                                    targets[l].extinguishFire(Boolean(playerName
                                        .slice(1)[i].split(":")[1]
                                        ?.toLowerCase()));
                                }
                                catch (e) {
        ...
        try {blockStatesFullList = String([String(blockStatesFullList), block.block.permutation.getAllStates()]).split(","); } catch(e){console.error(e, e.stack);}*/
            ...
            try {
                event.sourceEntity.onScreenDisplay.setActionBar("§eBlock States For §c" +
                    block.block.typeId +
                    "§e: §a\n" +
                    blockStatesFullList);
            }
        ...
        if (sourceType == "Entity") {message3 = Vector.add(sourceEntity.location, {x: Number(message2[1].split("~")[1]), y: Number(message2[1].split("~")[2]), z: Number(message2[1].split("~")[3])})} else { message3 = Vector.add(sourceBlock.location, {x: Number(message2[1].split("~")[1]), y: Number(message2[1].split("~")[2]), z: Number(message2[1].split("~")[3])}) }
            ...
            console.warn("RGB")*/
                            try {
                                currentMolangVariableMap.setColorRGB(message2[index].slice(12).split(":")[0], {
                                    blue: Number(message2[index]
                                        .slice(12)
                                        .split(":")[3]),
                                    green: Number(message2[index]
                                        .slice(12)
                                        .split(":")[2]),
                                    red: Number(message2[index]
                                        .slice(12)
                                        .split(":")[1]),
                                });
                            }
            ...
            console.warn(message2[index].slice(13).split(":")[0] + "," + Number(message2[index].slice(13).split(":")[4]) + "," + Number(message2[index].slice(13).split(":")[3]) + "," + Number(message2[index].slice(13).split(":")[2]) + "," + Number(message2[index].slice(13).split(":")[1]))*/
                ...
                console.warn("Vector3")*/
                                    try {
                                        currentMolangVariableMap.setVector3(message2[index]
                                            .slice(11)
                                            .split(":")[0], {
                                            z: Number(message2[index]
                                                .slice(11)
                                                .split(":")[3]),
                                            y: Number(message2[index]
                                                .slice(11)
                                                .split(":")[2]),
                                            x: Number(message2[index]
                                                .slice(11)
                                                .split(":")[1]),
                                        });
                                    }
                ...
                console.warn("Float")*/
                                        try {
                                            currentMolangVariableMap.setFloat(message2[index]
                                                .slice(2)
                                                .split(":")[0], Number(message2[index]
                                                .slice(2)
                                                .split(":")[1]));
                                        }
                                        catch (e) {
                ...
                console.warn("RGB")*/
                                            try {
                                                currentMolangVariableMap.setColorRGB(message2[index]
                                                    .slice(4)
                                                    .split(":")[0], {
                                                    blue: Number(message2[index]
                                                        .slice(4)
                ...
                console.warn("RGBA")*/
                                                try {
                                                    currentMolangVariableMap.setColorRGBA(message2[index]
                                                        .slice(5)
                                                        .split(":")[0], {
                                                        alpha: Number(message2[index]
                                                            .slice(5)
        ...
        try {blockStatesFullList = String([String(blockStatesFullList), block.block.permutation.getAllStates()]).split(","); } catch(e){console.error(e, e.stack);}*/
            try {
                world
                    ...
                    .spawnParticle(message2[0], message3, currentMolangVariableMap);
            }
            catch (e) {
                console.error(e, e.stack);
            }
