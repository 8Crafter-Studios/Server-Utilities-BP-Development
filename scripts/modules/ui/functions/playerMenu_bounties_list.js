...
export async function playerMenu_bounties_list(sourceEntitya, pagen = 0, maxplayersperpage = config.ui.pages
    ...
    let bounties = TotalBounty.getAll().map(b => [b, savedPlayer.getSavedPlayer("player:" + b.targetId)])
    ...
    let displayPlayers = bounties
        ...
        : true).sort(((a, b) => (a[0].totalValue > b[0].totalValue ? -1 : a[0].totalValue < b[0].totalValue ? 1 : a[1].name > b[1].name ? -1 : a[1].name < b[1].name ? 1 : 0)));
    const numsavedplayers = displayPlayers.length;
    form.title(`${!!search ? "Search Results" : "Bounties"} ${Math.min(numsavedplayers, page * maxplayersperpage + 1)}-${Math.min(numsavedplayers, (page + 1) * maxplayersperpage)} of ${numsavedplayers}`);
    ...
    return await forceShow(form, sourceEntity)
            ...
            case "search":
                {
                    ...
                    if (!!!rb || rb?.canceled == true) {
                        return await playerMenu_bounties_list(sourceEntity, page, maxplayersperpage, search);
                    }
                    return await playerMenu_bounties_list(sourceEntity, undefined, maxplayersperpage, {
        ...
        return await showMessage(sourceEntity as Player, undefined, "§cSorry, the search feature has not been implemented yet.", "Back", "Close").then(async r=>{
            if(r.selection==0){
                ...
                break;
            case "previous":
                return await playerMenu_bounties_list(sourceEntity, Math.max(0, page - 1), maxplayersperpage, search);
                break;
            case "next":
                return await playerMenu_bounties_list(sourceEntity, Math.min(numpages - 1, page + 1), maxplayersperpage, search);
                break;
            case "bounty":
                if ((await playerMenu_bounty(sourceEntity, bounties[r.selection - 3][0], bounties[r.selection - 3][1])) == 1) {
                    return await playerMenu_bounties_list(sourceEntity, page, maxplayersperpage, search);
                }
                else {
                    return 0;
