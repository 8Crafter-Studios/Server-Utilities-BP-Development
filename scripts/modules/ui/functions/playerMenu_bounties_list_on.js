...
export async function playerMenu_bounties_list_on(sourceEntitya, pagen = 0, maxplayersperpage = config.ui.pages
    ...
    let bounties = Bounty.getBountiesOnPlayer(sourceEntity.id).map(b => [b, b.getLinkedSourceSavedPlayer()])
    ...
    form.title(`${!!search ? "Search Results" : "Bounties Placed On Me"} ${Math.min(numsavedplayers, page * maxplayersperpage + 1)}-${Math.min(numsavedplayers, (page + 1) * maxplayersperpage)} of ${numsavedplayers}`);
    ...
    return await forceShow(form, sourceEntity)
            ...
            case "search":
                {
                    ...
                    if (!!!rb || rb?.canceled == true) {
                        return await playerMenu_bounties_list_on(sourceEntity, page, maxplayersperpage, search);
                    }
                    return await playerMenu_bounties_list_on(sourceEntity, undefined, maxplayersperpage, {
        ...
        return await showMessage(sourceEntity as Player, undefined, "§cSorry, the search feature has not been implemented yet.", "Back", "Close").then(async r=>{
            if(r.selection==0){
                ...
                break;
            case "previous":
                return await playerMenu_bounties_list_on(sourceEntity, Math.max(0, page - 1), maxplayersperpage, search);
                break;
            case "next":
                return await playerMenu_bounties_list_on(sourceEntity, Math.min(numpages - 1, page + 1), maxplayersperpage, search);
                break;
            case "bounty":
...
//# sourceMappingURL=playerMenu_bounties_list_on.js.map