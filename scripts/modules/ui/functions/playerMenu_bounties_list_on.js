...
export async function playerMenu_bounties_list_on(sourceEntitya, pagen = 0, maxbountiesperpage = config.ui.pages.maxPlayersPerManagePlayersPage ?? 9, search, cachedBounties) {
    ...
    if (cachedBounties === undefined) {
        let bounties = Bounty.getBountiesOnPlayer(sourceEntity.id)
            .map((b) => [b, b.getLinkedSourceSavedPlayer()])
    ...
    form.title(`${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Bounties Placed On Me"} ${Math.min(numsavedplayers, page * maxbountiesperpage + 1)}-${Math.min(numsavedplayers, (page + 1) * maxbountiesperpage)} of ${numsavedplayers}`);
    ...
    return await forceShow(form, sourceEntity)
        ...
        return await showMessage(sourceEntity as Player, undefined, "§cSorry, the search feature has not been implemented yet.", "Back", "Close").then(async r=>{
            ...
            case "bounty":
                if ((await playerMenu_bounty_on_individual(sourceEntity, displayBountiesB[r.selection - 6][0], displayBountiesB[r.selection - 6][1])) == 1) {
                    return await playerMenu_bounties_list_on(sourceEntity, page, maxbountiesperpage, search);
                }
                else {
                    return 0;
