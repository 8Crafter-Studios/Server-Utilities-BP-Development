...
export async function playerMenu_bounties_list_from(sourceEntitya, pagen = 0, maxbountiesperpage = config.ui.pages.maxPlayersPerManagePlayersPage ?? 9, search, cachedBounties) {
    ...
    if (cachedBounties === undefined) {
        let bounties = Bounty.getBountiesFromPlayer(sourceEntity.id)
            .map((b) => [b, b.getLinkedTargetSavedPlayer()])
        ...
        displayBounties = bounties
            ...
            : true)
            .sort((a, b) => a[0].value > b[0].value ? -1 : a[0].value < b[0].value ? 1 : a[1].name > b[1].name ? -1 : a[1].name < b[1].name ? 1 : 0);
    }
    const numsavedplayers = displayBounties.length;
    form.title(`${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Bounties Placed By Me"} ${Math.min(numsavedplayers, page * maxbountiesperpage + 1)}-${Math.min(numsavedplayers, (page + 1) * maxbountiesperpage)} of ${numsavedplayers}`);
    ...
    return await forceShow(form, sourceEntity)
            ...
            case "search":
                {
                    ...
                    return await playerMenu_bounties_list_from(sourceEntity, undefined, maxbountiesperpage, {
        ...
        return await showMessage(sourceEntity as Player, undefined, "§cSorry, the search feature has not been implemented yet.", "Back", "Close").then(async r=>{
            ...
            case "previous":
                return await playerMenu_bounties_list_from(sourceEntity, Math.max(0, page - 1), maxbountiesperpage, search, cachedBounties);
            case "go": {
                const rb = await tryget(async () => await new ModalFormData()
                    .title("Go To Page")
                    ...
                    .forceShow(sourceEntity));
                return await playerMenu_bounties_list_from(sourceEntity, Math.max(1, Math.min(numpages, rb.formValues?.[0]?.toNumber() ?? page + 1)) - 1, maxbountiesperpage, search, displayBounties);
            }
            case "next":
            ...
            case "bounty":
                if ((await playerMenu_bounty_from_individual(sourceEntity, displayBountiesB[r.selection - 6][0], displayBountiesB[r.selection - 6][1])) == 1) {
                    return await playerMenu_bounties_list_from(sourceEntity, page, maxbountiesperpage, search, cachedBounties);
                }
                else {
                    return 0;
            ...
            case "refresh":
                return await playerMenu_bounties_list_from(sourceEntity, page, maxbountiesperpage, search, undefined);
            case "back":
                return 1;
            case "close":
                return 0;
