...
export async function playerMenu_leaderboard(sourceEntitya, leaderboard, pagen = 0, maxplayersperpage = config.ui.pages
    ...
    form.title(`${!!search ? "Search Results" : leaderboard.menuTitle} ${Math.min(numsavedplayers, page * maxplayersperpage + 1)}-${Math.min(numsavedplayers, (page + 1) * maxplayersperpage)} of ${numsavedplayers}`);
    ...
    return await forceShow(form, sourceEntity)
            ...
            case "search":
                {
                    ...
                    if (!!!rb || rb?.canceled == true) {
                        return await playerMenu_leaderboard(sourceEntity, leaderboard, page, maxplayersperpage, search);
                    }
        ...
        return await showMessage(sourceEntity as Player, undefined, "§cSorry, the search feature has not been implemented yet.", "Back", "Close").then(async r=>{
            if(r.selection==0){
                ...
                break;
            case "previous":
                return await playerMenu_leaderboard(sourceEntity, leaderboard, Math.max(0, page - 1), maxplayersperpage, search);
                break;
            case "next":
                return await playerMenu_leaderboard(sourceEntity, leaderboard, Math.min(numpages - 1, page + 1), maxplayersperpage, search);
                break;
            case "player":
