...
import { addIDBanOnPlayer } from "./manageBans";
/**
 ...
 */
export async function moderationMenu_quickBan(sourceEntity) {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    while (true) {
        try {
            const form = new ActionFormData();
            form.title(customFormUICodes.action.titles.formStyles.medium + "Ban Online Player");
            ...
            if (playerslist.length === 0) {
                form.body("No players available to ban.");
            }
...
//# sourceMappingURL=moderationMenu_quickBan.js.map