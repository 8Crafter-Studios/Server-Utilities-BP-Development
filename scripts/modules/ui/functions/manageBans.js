...
export async function manageBans(sourceEntity, pagen = 0, maxentriesperpage = config.ui.pages.maxPlayersPerManagePlayersPage ?? 9, search, cachedEntries) {
    ...
    while (true) {
        ...
        form.title(`${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : ""} ${Math.min(numentries, page * maxentriesperpage + 1)}-${Math.min(numentries, (page + 1) * maxentriesperpage)} of ${numentries}`);
...
export async function manageBan(sourceEntity, ban) {
    ...
    while (true) {
        try {
            ...
            form.body(`§bformat_version: §e${ban.format_version}\n§r§bban_format_version: §e${ban.ban_format_version}\n§r§bbanId: §6${ban.banId}\n§r§btype: §a${ban.type}\n§r§bBan Duration: §q${isPermanent ? "Permanent" : `${duration.days}d, ${duration.hours}h ${duration.minutes}m ${duration.seconds}s ${duration.milliseconds}ms`}${isPermanent
                ? ""
...
export async function unbanPlayer(sourceEntity, selectedBan) {
    ...
    try {
        ...
        switch (["cancel", "unban"][r.selection]) {
            ...
            case "unban": {
                selectedBan.remove();
                return ((await showMessage(player, "Player Unbanned", `${selectedBan.playerName ?? selectedBan.originalPlayerName ?? "Unknown Name"}<${selectedBan.playerId ?? selectedBan.originalPlayerId ?? "Unknown ID"}> has been sucessfully unbanned.`, "Back", "Close")).selection !== 1).toNumber();
            }
        }
    }
    catch (e) {
    ...
    while (true) {
        try {
            ...
            ban.saveBan({
                ...
                unbanDate: r.formValues[1].trim() === "" ? Infinity : parseDurationRelative(r.formValues[1], banDate) * 60000 + Date.now(),
                format_version: format_version,
