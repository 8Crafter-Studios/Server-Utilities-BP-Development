...
export async function playerMenu_warps(sourceEntitya) {
    ...
    return await forceShow(form, sourceEntity)
        ...
        switch ((!!warps[r.selection] ? "warp" : undefined) ??
            cullUndefined([sourceEntity.hasTag("admin") ? "newWarp" : undefined, "back", "close"])[r.selection - warps.length]) {
            case "warp":
                ...
                if (sourceEntity.dimension !== dimensionsb[warp.dimension] && !config.homeSystem.allowCrossDimensionalTeleport) {
                    if ((await showMessage(sourceEntity, "Error", `§cSorry but cross-dimensional home teleports have been disabled.`, "Back", "Close"))
                        .selection === 0) {
                        return await playerMenu_warps(sourceEntity);
                    }
                    else {
            ...
            case "newWarp": {
                if (!sourceEntity.hasTag("admin")) {
                    if ((await showMessage(sourceEntity, "Error", `§cSorry but you do not have permission to create a new warp.`, "Back", "Close"))
                        .selection === 0) {
                        return await playerMenu_warps(sourceEntity);
                    }
                    else {
                ...
                config.warpsSystem.warps = [
                    ...config.warpsSystem.warps,
                    {
                        dimension: dimensionsd[r.formValues?.[2]],
                        displayName: r.formValues?.[0],
                        location: coordinatesB(r.formValues?.[1], sourceEntity.location, sourceEntity.getViewDirection()),
                        icon: r.formValues?.[3] !== "" ? r.formValues?.[3] : undefined,
                    },
                ];
                return await playerMenu_warps(sourceEntity);
            }
