...
export async function generalSettings(sourceEntity) {
    ...
    try {
        ...
        const formOptionsMap = {
            ...
            gametestStructureDefaultSpawnLocation: () => form.textField("§l§fgametestStructureDefaultSpawnLocation§r§f\nThe default spawn location for the gametest structures, this is used when spawning in no AI entities and simulated players", "x y z", { defaultValue: vTStr(config.gametestStructureDefaultSpawnLocation) }),
            spawnCommandLocation: () => form.textField("§l§fspawnCommandLocation§r§f\nThe location to teleport players when they use the \\spawn command, it is a list of coordinates separated by spaces, leaving it blank will disable the spawn command", "x y z", { defaultValue: cullEmpty([config.spawnCommandLocation.x, config.spawnCommandLocation.y, config.spawnCommandLocation.z]).join(" ") }),
            spawnCommandDimension: () => form.dropdown("§l§fspawnCommandDimension§r§f\nThe dimension to teleport players when they use the \\spawn command, it is a list of coordinates separated by spaces, the default is overworld", ["§aOverworld", "§cNether", "§dThe End"], { defaultValueIndex: dimensionsd.indexOf(config.spawnCommandLocation.dimension.id) }),
            invalidChatCommandAction: () => form.dropdown("§l§finvalidChatCommandAction§r§f\nWhat to do when a chat command is typed that does not exist, or that the player does not have permission to use. ", ["Do Nothing", "Send Message", "Cancel Message", "Warn Player"], { defaultValueIndex: config.invalidChatCommandAction }),
