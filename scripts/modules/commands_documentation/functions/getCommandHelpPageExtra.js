...
export function getCommandHelpPageExtra(commandName, player) {
     ...
     */
    let cmd = CommandRegistry.getCommand(commandName, { typeFilter: "built-in", includeAliases: true }) ??
    ...
    if (cmd instanceof command) {
        ...
        return `§e${cmd.command.name}${(cmd.command.aliases?.length ?? 0) != 0
            ...
            cmd.command.name, currentCommandName))}${!cmd.command.flagsDocs ? "" : "\nFlags:\n" + cmd.command.flagsDocs.split("\n").join("§r\n")}${"\nVersion: " + cmd.command.formatting_code + cmd.command.command_version}${!cmd.command.categories ? "" : "§r\nCategories: " + JSON.stringify(cmd.command.categories)}${cmd.command.settings?.defaultSettings === undefined
            ? ""
            : "§r\nDefault Required Tags: " + JSONB.stringify(cmd.command.settings.defaultSettings.requiredTags)}${cmd.command.settings?.requiredTags === undefined ? "" : "§r\nRequired Tags: " + JSON.stringify(cmd.command.settings.requiredTags)}${cmd.command.settings?.requiresOp === undefined ? "" : "§r\nRequires OP: " + JSON.stringify(cmd.command.settings.requiresOp)}${cmd.command.settings?.requiredPermissionLevel === undefined
            ? ""
            : "§r\nRequired Permission Level: " + JSON.stringify(cmd.command.settings.requiredPermissionLevel)}§r\nType: ${cmd.command.type}§r\n${cmd.command.settings?.enabled === false ? "§cDISABLED" : "§aENABLED"}${!cmd.command.deprecated ? "" : "\n§cThis command is deprecated!"}${cmd.command.functional ? "" : "\n§cThis command is not functional!"}${player ? (cmd.command.playerCanExecute(player) ? "" : "\n§cYou do not have permission to use this command!") : ""}§r${cmd.command.type === "built-in"
