...
export function generateNBTFileB(location, nbt) {
    nbt.blocks.forEach((b) => tryrun(() => (location.dimension.setBlockType(Vector3Utils.add(location, {
        ...
        y: b.pos[1],
        z: b.pos[2],
    }), nbt.palette[b.state].Name.replace("minecraft:active - lit_redstone_lamp", "minecraft:light_gray_terracotta").replace("minecraft:coarse_dirt", "minecraft:dirt")),
        nbt.palette[b.state].Name == "minecraft:coarse_dirt" ||
            nbt.palette[b.state].Name == "coarse_dirt"
            ? location.dimension.setBlockPermutation(Vector3Utils.add(location, {
                x: b.pos[0],
                y: b.pos[1],
                z: b.pos[2],
