...
/**
 * Fills the walls in the specified area. Supports block masks.
 * @async
 ...
 * @remarks The most modern version of the wall block filling functions.
 */
export async function fillWalls(begin, end, dimension, block, options) {
                ...
                if (integrity != 100) {
                    ...
                    for (let x = Math.min(begin.x, end.x); x <= Math.max(begin.x, end.x); x++) {
                        for (let y = Math.min(begin.y, end.y); y <= Math.max(begin.y, end.y); y++) {
                            for (let z = Math.min(begin.z, end.z); z <= Math.max(begin.z, end.z); x == begin.x || x == end.x || z == end.z ? z++ : (z = end.z)) {
...
//# sourceMappingURL=fillWalls.js.map