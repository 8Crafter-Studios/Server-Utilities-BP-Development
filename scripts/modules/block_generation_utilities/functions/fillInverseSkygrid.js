 ...
 */
export async function fillInverseSkygrid(begin, end, gridSize, dimension, block, options) {
    const startX = Math.floor(begin.x);
    const startY = Math.floor(begin.y);
    const startZ = Math.floor(begin.z);
...
//# sourceMappingURL=fillInverseSkygrid.js.map