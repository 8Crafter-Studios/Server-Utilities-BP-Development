 ...
 * @param {Vector3} stretch The amount to stretch each axis of the sphere.
 ...
 * @remarks The most modern version of the stretched sphere block filling functions.
 */
export async function fillStretchedSphere(center, radius, stretch, dimension, block, options) {
    ...
    const stretchedRadius = Vector.scale(stretch, radius);
        ...
        if (!!!options?.blockMask || options?.blockMask?.blocks?.length == 0) {
            ...
            if (replacemode) {
                ...
                if (integrity != 100) {
                    ...
                    for (let x = centerX - Math.floor(stretchedRadius.x); x <= centerX + Math.ceil(stretchedRadius.x); x++) {
                        for (let y = centerY - Math.floor(stretchedRadius.y); y <= centerY + Math.ceil(stretchedRadius.y); y++) {
                            for (let z = centerZ - Math.floor(stretchedRadius.z); z <= centerZ + Math.ceil(stretchedRadius.z); z++) {
                                const distanceSquared = Math.pow((x - centerX) / stretch.x, 2) +
                                    Math.pow((y - centerY) / stretch.y, 2) +
                                    Math.pow((z - centerZ) / stretch.z, 2);
...
//# sourceMappingURL=fillStretchedSphere.js.map