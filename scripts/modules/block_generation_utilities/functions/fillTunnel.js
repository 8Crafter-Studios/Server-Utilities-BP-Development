 ...
 */
export async function fillTunnel(center, radius, length, axis, dimension, block, options) {
    ...
    const isWithinCylinderWall = (x, y, z) => {
        switch (axis) {
            case 'x':
                return Math.pow(y - centerY, 2) + Math.pow(z - centerZ, 2) <= Math.pow(radius, 2) &&
                    Math.pow(y - centerY, 2) + Math.pow(z - centerZ, 2) >= Math.pow(radius - 1, 2) &&
                    Math.abs(x - centerX) <= length / 2;
            case 'y':
                return Math.pow(x - centerX, 2) + Math.pow(z - centerZ, 2) <= Math.pow(radius, 2) &&
                    Math.pow(x - centerX, 2) + Math.pow(z - centerZ, 2) >= Math.pow(radius - 1, 2) &&
                    Math.abs(y - centerY) <= length / 2;
            case 'z':
                return Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2) <= Math.pow(radius, 2) &&
                    Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2) >= Math.pow(radius - 1, 2) &&
                    Math.abs(z - centerZ) <= length / 2;
            default:
                throw new Error(`Invalid axis: ${axis}`);
        }
    };
    const fillBlock = async (x, y, z) => {
        if (isWithinCylinderWall(x, y, z)) {
...
//# sourceMappingURL=fillTunnel.js.map