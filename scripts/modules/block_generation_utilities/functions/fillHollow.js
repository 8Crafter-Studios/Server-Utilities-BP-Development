 ...
 * @remarks The most modern version of the hollow block filling functions.
 */
export async function fillHollow(begin, end, dimension, block, options) {
                ...
                if (integrity != 100) {
                    ...
                    for (let x = Math.min(begin.x, end.x); x <= Math.max(begin.x, end.x); x++) {
                        for (let y = Math.min(begin.y, end.y); y <= Math.max(begin.y, end.y); y++) {
                            for (let z = Math.min(begin.z, end.z); z <= Math.max(begin.z, end.z); x == begin.x || x == end.x || y == begin.y || y == end.y || z == end.z ? z++ : (z = end.z)) {
...
//# sourceMappingURL=fillHollow.js.map