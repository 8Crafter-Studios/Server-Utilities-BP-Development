 ...
 * @returns
 */
export async function editAreasForCustomCategory(player, prefix) {
    ...
    areas.forEach((a, i) => {
        /*console.warn(aelement.slice(22)); */ form.button(customFormUICodes.action.buttons.positions.main_only + a.id, a.icon_path ?? "textures/ui/area_xyz");
                ...
                return await forceShow(form, player).then(async (w) => {
                    ...
                    switch (w.selection) {
                        case 0: {
                            ...
                            return await forceShow(form, player).then(async (q) => {
                                ...
                                if (defaults.dimension === dimension) {
                                    ProtectedAreas.areas.advancedArea[prefix][dimensionse[dimension]].splice(ProtectedAreas.areas.advancedArea[prefix][dimensionse[dimension]].findIndex((a) => a.id === defaults.id), 1, {
                                        id: defaults.id,
                                        ...newValue,
                                    });
                                }
                                else {
                                    ProtectedAreas.areas.advancedArea[prefix][dimensionse[defaults.dimension]].splice(ProtectedAreas.areas.advancedArea[prefix][dimensionse[defaults.dimension]].findIndex((a) => a.id === defaults.id), 1);
                                    ProtectedAreas.areas.advancedArea[prefix][dimensionse[dimension]].push({
                                        id: defaults.id,
                                        ...newValue,
                                    });
                                }
                        ...
                        case 1:
                            ...
                            ProtectedAreas.areas.advancedArea[prefix][dimensionse[areas[t.selection].dimension]].splice(ProtectedAreas.areas.advancedArea[prefix][dimensionse[areas[t.selection].dimension]].findIndex((a) => a.id === areas[t.selection].id), 1);
                            return await editAreasForCustomCategory(player, prefix);
                        case 2:
                            return await editAreasForCustomCategory(player, prefix);
                        case 3:
