    ...
    if (event.itemStack?.typeId === "andexdb:selection_tool") {
        ...
        if (event.isFirstEvent) {
            ...
            if ([
                ...
                event.cancel = true;
                try {
                    srun(() => event.player.inventory.container.addItem(event.block.getItemStack() ??
            ...
            if ([
                ...
                event.cancel = true;
                try {
                    srun(() => event.player.inventory.container.addItem(event.block.getItemStack(undefined, true) ??
                        tryget(() => new ItemStack(event.block.typeId))));
                    return;
                }
                catch (e) {
