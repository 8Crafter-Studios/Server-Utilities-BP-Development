    ...
    if (config.worldBorder.overworld.enabled) {
        ...
        world
            ...
            if (intensity != 0 && !Number.isNaN(intensity)) {
                if (Date.now() >= outsideBorderTintShownTimes[p.id] + Number(p.getDynamicProperty("outsideBorderTintShowIntervalMS") ?? 500) ||
            ...
            if (!p.hasTag("canBypassWorldBorder") &&
                ...
                if (borderSettings.mode == 0) {
                    ...
                    if (p.location.x > borderSettings.to.x + borderSettings.buffer ||
            ...
            if (borderSettings.showBorderParticles) {
                ...
                if ((p.getDynamicProperty("useShadersCompatibleParticles") ?? false) == true || borderSettings.useShadersCompatibleBorderParticles) {
                    if (Math.abs(borderSettings.to.x - p.location.x) <= 50 &&
                        ...
                        try {
                            p.spawnParticle("andexdb:rising_border_dust_particle_b", {
                                ...
                                z: Math.max(Math.min(p.location.z + (Math.random() * borderZDistance - borderZDistance / 2) + borderZOffset, borderSettings.to.z), borderSettings.from.z),
                            });
                        }
                        catch (e) {
                    ...
                    if (Math.abs(borderSettings.to.z - p.location.z) <= 50 &&
                        ...
                        try {
                            p.spawnParticle("andexdb:rising_border_dust_particle_b", {
                                ...
                                y: Math.random() < 0.75 ? p.location.y + (Math.random() * 10 - 5) : p.location.y + (Math.random() * 100 - 50),
                                x: Math.max(Math.min(p.location.x + (Math.random() * borderXDistance - borderXDistance / 2) + borderXOffset, borderSettings.to.x), borderSettings.from.x),
                            });
                        }
                        catch (e) {
                            if (e instanceof LocationInUnloadedChunkError) {
                    ...
                    if (Math.abs(borderSettings.from.x - p.location.x) <= 50 &&
                        !(p.location.z > borderSettings.to.z + 50) &&
                        !(p.location.z < borderSettings.from.z - 50)) {
                        try {
                            p.spawnParticle("andexdb:rising_border_dust_particle_b", {
                    ...
                    if (Math.abs(borderSettings.from.z - p.location.z) <= 50 &&
                        !(p.location.x > borderSettings.to.x + 50) &&
                        !(p.location.x < borderSettings.from.x - 50)) {
                        try {
                            p.spawnParticle("andexdb:rising_border_dust_particle_b", {
