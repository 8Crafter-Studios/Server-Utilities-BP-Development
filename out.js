  ...
  var FirstPassVisitor = {
    ...
    Assertion({ node, parent, key, container, root, remove, replaceWith }, state) {
      ...
      if (kind === f.grapheme_boundary) {
        ...
        replaceWith(setParent(parseFragment(r3`(?=\z|\n)`), parent));
      } else if (kind === f.line_start) {
        replaceWith(setParent(parseFragment(r3`(?<=\A|\n(?!\z))`, { skipLookbehindValidation: true }), parent));
        ...
        replaceWith(setParent(parseFragment(r3`(?=\n?\z)`), parent));
...
export {JavaScriptScanner, createJavaScriptRawEngine, createJavaScriptRegexEngine, defaultJavaScriptRegexConstructor} from exports;
