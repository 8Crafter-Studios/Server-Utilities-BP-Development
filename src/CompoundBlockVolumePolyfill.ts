...
export class CompoundBlockVolume {
    ...
    private volumeToAABB(vol: BlockVolume): BlockBoundingBox {
        const min = vol.getMin();
        const max = vol.getMax();
        return { min, max };
    }

    private aabbTouchesOrOverlaps(a: BlockBoundingBox, b: BlockBoundingBox): boolean {
    ...
    private mergeAABB(a: BlockBoundingBox, b: BlockBoundingBox): BlockBoundingBox {
        return {
            min: {
    ...
    private mergeAABBs(aabbs: BlockBoundingBox[]): BlockBoundingBox[] {
        if (aabbs.length === 0) return [];

        // Simple O(n^2) merging; n is number of volumes, not blocks
        const result: BlockBoundingBox[] = [];
    ...
    getBlockLocationIterator(): BlockLocationIterator {
        const addAABBs: BlockBoundingBox[] = [];
        const subAABBs: BlockBoundingBox[] = [];
        ...
        let x = 0,
            y = 0,
            z = 0;
        let current: BlockBoundingBox | undefined;

        const isInsideSub = (pos: Vector3): boolean => {
        ...
        const iterator: BlockLocationIterator = {
            [Symbol.iterator]() {
                return this;
            },
