...
export function editAreas(player: Player, prefix: string){
    ...
    forceShow(form1234, player).then((t)=>{if (t.canceled) {editAreasMainMenu(player); return}; switch(true){
    ...
    forceShow(form12345, player).then((q)=>{if (q.canceled) {editAreas(player, prefix); return}; const [ id, from, to, dimension, mode, icon_path ] = (q as ModalFormResponse).formValues; world.setDynamicProperty("v2:"+prefix+id, JSON.stringify({from: {x: Number(String(from).split(",")[0]), y: Number(String(from).split(",")[1]), z: Number(String(from).split(",")[2])}, to: {x: Number(String(to).split(",")[0]), y: Number(String(to).split(",")[1]), z: Number(String(to).split(",")[2])}, dimension: Number(dimension), mode: Number(mode), icon_path: ((icon_path??"")=="")?undefined:icon_path})); })
    break; 
    ...
    forceShow(form1234567, player).then((w)=>{if (w.canceled) {editAreas(player, prefix); return}; switch((w as ActionFormResponse).selection){
    case 0: {
    ...
    forceShow(form123456, player).then((q)=>{if (q.canceled) {editAreas(player, prefix); return}; const [ from, to, dimension, mode, icon_path ] = (q as ModalFormResponse).formValues; world.setDynamicProperty(a[(t as ActionFormResponse).selection], JSON.stringify({from: {x: Number(String(from).split(",")[0]), y: Number(String(from).split(",")[1]), z: Number(String(from).split(",")[2])}, to: {x: Number(String(to).split(",")[0]), y: Number(String(to).split(",")[1]), z: Number(String(to).split(",")[2])}, dimension: Number(dimension), mode: Number(mode), icon_path: ((icon_path??"")=="")?undefined:icon_path})); })
    }
    break; 
    case 1: 
...
export function convertToCompoundBlockVolume(selection: String){let compoundFullBlockVolumes = new CompoundBlockVolume({x: 0, y: 0, z: 0}); let blockVolumeAllLists: {from: {x: number, y: number, z: number}, to: {x: number, y: number, z: number}}[]; blockVolumeAllLists = []; selection.split("|").forEach((selectionSection)=>{blockVolumeAllLists.push({from: {x: Math.min(Number(selectionSection.split(",")[0]), Number(selectionSection.split(",")[3])), y: Math.min(Number(selectionSection.split(",")[1]), Number(selectionSection.split(",")[4])), z: Math.min(Number(selectionSection.split(",")[2]), Number(selectionSection.split(",")[5]))}, to: {x: Math.max(Number(selectionSection.split(",")[0]), Number(selectionSection.split(",")[3])), y: Math.max(Number(selectionSection.split(",")[1]), Number(selectionSection.split(",")[4])), z: Math.max(Number(selectionSection.split(",")[2]), Number(selectionSection.split(",")[5]))}})}); return blockVolumeAllLists}
