 ...
 * @version 1.1.1
 */
globalThis.formatDateTime = function formatDateTime<includeMs extends boolean>(
    date: Date,
    timeZoneOffset: number = 0,
    includeMs: includeMs | boolean = false
): includeMs extends unknown
 ...
 * @version 1.0.0
 */
globalThis.formatDate = function formatDate(
    date: Date,
    timeZoneOffset: number = 0
) {
    const dateb = new Date(date.valueOf() + timeZoneOffset * 3600000);
 ...
 * @returns {any} The parsed JSON data.
 */
globalThis.JSONParseOld = function JSONParseOld(
    text: string,
    ...
    let a = JSON.parse(
        text
            .replace(
                /(?<="(?:\s*):(?:\s*))"{{(Infinity|NaN|-Infinity|undefined)}}"(?=(?:\s*)[,}](?:\s*))/g,
                '"{{\\"{{$1}}\\"}}"'
            )
            .replace(
                /(?<="(?:\s*):(?:\s*))(Infinity|NaN|-Infinity|undefined)(?=(?:\s*)[,}](?:\s*))/g,
    ...
    });
    {
        ...
        b.filter(
            (b) =>
                !!String(b[1]).match(
                    /^{{"{{(Infinity|NaN|-Infinity|undefined)}}"}}$/
                )
        ).forEach((v, i) => {
            console.log(v, i);
 ...
 * @returns {any} The JSON string.
 */
globalThis.JSONStringifyOld = function JSONStringifyOld(
    value: any,
...
globalThis.pfsend = function pfsend(
    player: Player | executeCommandPlayerW,
    value: any,
    space?: string | number
) {
...
globalThis.waitTicks = async function waitTicks(
    ...
    return new Promise((resolve) =>
        system.runTimeout(() => resolve(void null), ticks)
    );
};
    ...
    return Object.entries(base).every((v) =>
        Object.keys(objectToTest).includes(v[0])
            ? v[1].startsWith("[object ")
                ...
                : typeof Object.entries(objectToTest).find(
                      (c) => c[0] == v[0]
                  )[1] == v[1]
            : false
    );
