import { system, world, EntityDamageCause, LocationInUnloadedChunkError } from "@minecraft/server";
...
repeatingIntervals.worldBorderSystem = system.runInterval(() => {
    if (config.worldBorder.overworld.enabled) {
        const borderSettings = config.toJSON(config.worldBorder.overworld)  as typeof config.worldBorder.overworld;
        world
            .getAllPlayers()
            ...
            .forEach((p) => {
                ...
                if (intensity != 0 && !Number.isNaN(intensity)) {
                    if (
                        Date.now() >= outsideBorderTintShownTimes[p.id]! + Number(p.getDynamicProperty("outsideBorderTintShowIntervalMS") ?? 500) ||
                ...
                if (
                    ...
                    if (borderSettings.mode == 0) {
                        ...
                        if (
                            p.location.x > borderSettings.to.x + borderSettings.buffer ||
                            p.location.z > borderSettings.to.z + borderSettings.buffer ||
                            p.location.x < borderSettings.from.x - borderSettings.buffer ||
                            p.location.z < borderSettings.from.z - borderSettings.buffer
                        ) {
                ...
                if (borderSettings.showBorderParticles) {
                    ...
                    if ((p.getDynamicProperty("useShadersCompatibleParticles") ?? false) == true || borderSettings.useShadersCompatibleBorderParticles) {
                        if (
                            Math.abs(borderSettings.to.x - p.location.x) <= 50 &&
                            ...
                            try {
                                p.spawnParticle("andexdb:rising_border_dust_particle_b", {
                                    ...
                                    y: Math.random() < 0.75 ? p.location.y + (Math.random() * 10 - 5) : p.location.y + (Math.random() * 100 - 50),
                                    z: Math.max(
                                        Math.min(p.location.z + (Math.random() * borderZDistance - borderZDistance / 2) + borderZOffset, borderSettings.to.z),
                        ...
                        if (
                            ...
                            try {
                                p.spawnParticle("andexdb:rising_border_dust_particle_b", {
                                    ...
                                    y: Math.random() < 0.75 ? p.location.y + (Math.random() * 10 - 5) : p.location.y + (Math.random() * 100 - 50),
                                    x: Math.max(
                                        Math.min(p.location.x + (Math.random() * borderXDistance - borderXDistance / 2) + borderXOffset, borderSettings.to.x),
                        ...
                        if (
                            Math.abs(borderSettings.from.x - p.location.x) <= 50 &&
                            !(p.location.z > borderSettings.to.z + 50) &&
                            !(p.location.z < borderSettings.from.z - 50)
                        ) {
                            try {
    ...
    if (config.worldBorder.nether.enabled) {
        const borderSettings = config.toJSON(config.worldBorder.nether)  as typeof config.worldBorder.nether;
        world
            .getAllPlayers()
    ...
    if (config.worldBorder.the_end.enabled) {
        const borderSettings = config.toJSON(config.worldBorder.the_end)  as typeof config.worldBorder.the_end;
        world
            .getAllPlayers()
