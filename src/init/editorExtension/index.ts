...
registerEditorExtension(
    "8CrafterServerUtilities",
    function SUEditorExtensionActivation(uiSession) {
        //#region Theme
        const switchBackToServerUtilitiesTheme: boolean = uiSession.extensionContext.settings.theme.getCurrentTheme() === serverUtilitiesThemeID;
        ...
        if (switchBackToServerUtilitiesTheme) uiSession.extensionContext.settings.theme.setCurrentTheme(serverUtilitiesThemeID);
        ...
        // });
        //#endregion
        const scriptEvalPropertyPane: IRootPropertyPane = uiSession.createPropertyPane({ title: "Execute Script", uniqueId: "ScriptEval" });
        const scriptEvalOverlayPane: IModalOverlayPane = scriptEvalPropertyPane.createModalOverlayPane({ title: "Execute Script Modal" });
        let scriptEvalScriptContents: string = "";
        // TODO
        scriptEvalOverlayPane.contentPane.addString("a", {
            title: "JavaScript",
            onChange(newValue: string, _oldValue: string): void {
        ...
        uiSession.menuBar.getMenu(CoreMenuType.Edit).then((menu: IMenu): void => {
            const flipSelectionVerticalAction = uiSession.actionManager.createAction({
                actionType: ActionTypes.NoArgsAction,
                async onExecute(): Promise<void> {
                    const volume: RelativeVolumeListBlockVolume = uiSession.extensionContext.selectionManager.volume.get();
                    const bounds: BlockBoundingBox = volume.getBoundingBox();
                    const volumeList: BlockVolume[] = volume.getVolumeList();
                    ...
                    for (const blockVolume of volumeList) {
                        ...
                        const a: number = bounds.min.y + bounds.max.y - originalTo.y;
                        const b: number = bounds.min.y + bounds.max.y - originalFrom.y;
    ...
    function SUEditorExtensionShutdown(uiSession): void {
