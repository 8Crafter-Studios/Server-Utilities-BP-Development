...
subscribedEvents.afterPlayerJoin = world.afterEvents.playerJoin.subscribe((event) => {
    ...
    (async () => {
        ...
        if (
            !!(
                ban?.getValidBans()?.idBans?.find((_) => _?.playerId == event?.playerId) ??
            ...
            try {
                ...
                let b =
                    ban
                        ?.getValidBans()
                        ?.idBans?.sort((a: ban, b: ban) => 1 - 2 * Number(a?.banDate > b?.banDate))
                        ?.find((_) => _?.playerId == event?.playerId) ??
                    ban
                        .getValidBans()
                        .nameBans?.sort((a: ban, b: ban) => 1 - 2 * Number(a?.banDate > b?.banDate))
                        ...
                        tryget(() =>
                            String(
                                eval(b?.reason!)
                                    ...
                                    ?.replaceAll("{timeRemainingRaw}", String(b?.timeRemainingRaw)) ?? b?.reason
                            )
                        ) ?? b?.reason;
                } catch (e) {
                    reason =
