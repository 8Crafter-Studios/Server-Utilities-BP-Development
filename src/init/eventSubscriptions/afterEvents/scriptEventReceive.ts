    ...
    system.afterEvents.scriptEventReceive.subscribe((event) => {
        ...
        if (id == "andexdb:blueModsInitSignal") {
            world
                .getDimension("overworld")
                .runCommand(
                    `/scriptevent bluemods:blueModsInitSignalReceivedByDebugSticks ${format_version}`
                );
            ...
            world
                .getDimension("overworld")
                .runCommand(
                    `/scriptevent bluemods:blueModsTestSignalReceivedByDebugSticks ${format_version}`
                );
            if (bluemods_anticheat_format_version != null &&
                message.trim() != entity_scale_format_version) {
                globalThis.multipleBlueModsAnticheatVersionsDetected = true;
            ...
            if (message.startsWith("players:") &&
                ...
                form2
                    ...
                    .then((t) => {
                        if (t.canceled) return;
                        let [
                            playerViewer, selectionType, entityUUID, blockLocationIndex, blockLocationCoordinates,
                        ] = t.formValues;
                        let playerViewerB = Number(playerViewer);
                        ...
                        if (selectionType == 1) {
                            playerTargetB = world
                                ...
                                .find(
                                    (entityValue) => entityValue.id == entityUUID
                                );
                        }
                        if (selectionType == 4) {
                            playerTargetB = world
...
break;*/ /*
        ...
        if (id == "andexdb:itemLoreInventoryModifier" ||
            ...
            form2
                ...
                .then((t) => {
                    if (t.canceled) return;
                    let [
                        slotNumber, slotType, playerTarget, playerViewer, debug2,
                    ] = t.formValues;
                    let playerTargetB = Number(playerTarget);
                    let playerViewerB = Number(playerViewer);
                    let inventory: any;
    ...
    form.textField("Item Data", "Trigger Event", "");*/
                    ...
                    form.show(players[playerViewerB] as any)
                        .then((r) => {
                            ...
                            // This will assign every input their own variable
                            let [
                                itemName, itemLore, canDestroy, canPlaceOn, triggerEvent, setAmount, amount, keepOnDeath, lockMode, setLore, clearLore, newItem, newItemType, newItemCount /*, newItemData*/, moveItem, moveFromSlot, moveToSlot, moveFromContainerType, moveFromContainer, moveFromContainerBlock, moveToContainerType, moveToContainer, moveToContainerBlock, swapItems, swapSlot, swapOtherSlot, swapContainerType, swapContainer, swapContainerBlock, swapOtherContainerType, swapOtherContainer, swapOtherContainerBlock, transferItem, transferFromSlot, transferFromContainerType, transferFromContainer, transferFromContainerBlock, transferToContainerType, transferToContainer, transferToContainerBlock, debug,
                            ] = r.formValues; /*
    console.warn(r.formValues);*/
    ...
    try{ durability2.damage = Number(10); } catch(e){if (Boolean(debug2) == true) { console.error(e, e.stack)}; }*/
                ...
                moveFromSlotB = undefined*/
                                ...
                                moveFromContainerB = players[Number(moveFromContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                                ...
                                moveToContainerB = players[Number(moveToContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                ...
                moveFromSlotB = undefined*/
                                ...
                                let mode = 0;
                                swapContainerB = players[Number(swapContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                                let itemA: any;
                                itemA = undefined;
                                if (Number(swapSlot) > 35 &&
                                    ...
                                    try {
                                        swapContainerB = players[playerTargetB].getComponent(
                                            "equippable"
                                        ) as EntityEquippableComponent;
                                        ...
                                        itemA = swapContainerB
                                            .getEquipment(
                                                equipmentPlayerSlotsList[Number(swapSlot)]
                                            )
                                            .clone();
                                    } catch (e) {
                                        if (Boolean(debug2) == true) {
                                ...
                                swapOtherContainerB = players[Number(swapOtherContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                                let itemB: any;
                                itemB = undefined;
                                if (Number(swapOtherSlot) > 35) {
                                    try {
                                        swapOtherContainerB = players[playerTargetB].getComponent(
                                            "equippable"
                                        ) as EntityEquippableComponent;
                                        swapOtherSlot =
                                        ...
                                        itemB = swapOtherContainerB
                                            .getEquipment(
                                                equipmentPlayerSlotsList[Number(swapOtherSlot)]
                                            )
                                            .clone();
                                    } catch (e) {
                                        if (Boolean(debug2) == true) {
                ...
                moveFromSlotB = undefined*/
                                ...
                                transferFromContainerB = players[Number(transferFromContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                                ...
                                transferToContainerB = players[Number(transferToContainer)].getComponent(
                                    "inventory"
                                ) as EntityInventoryComponent;
                ...
                targetList = String([String(targetList), players[index].nameTag]).split(",");
                                            ...
                                            form3
                                                ...
                                                .then((t) => {
                                                    if (t.canceled) return;
                                                    let [
                                                        transferType, fromBlockSelectionMode, fromBlockPosition, toBlockSelectionMode, toBlockPosition, debug2,
                                                    ] = t.formValues;
                                                    ...
                                                    let fromBlockPositionC = fromBlockPositionB.getComponent(
                                                        "inventory"
                                                    ) as BlockInventoryComponent;
                                                    let toBlockPositionC = toBlockPositionB.getComponent(
                                                        "inventory"
                                                    ) as BlockInventoryComponent;
                                                    if (Number(
                                        ...
                                        case 1:
                                            ...
                                            form2
                                                ...
                                                .then((t) => {
                                                    if (t.canceled) return;
                                                    let [
                                                        transferType, playerTarget, playerViewer, debug2,
                                                    ] = t.formValues;
                                                    let playerTargetB = Number(playerTarget);
                                                    let playerViewerB = Number(playerViewer);
                                                    const fromInventory = players[playerTargetB].getComponent(
                                                        "inventory"
                                                    ) as EntityInventoryComponent;
                                                    const toInventory = players[playerViewerB].getComponent(
                                                        "inventory"
                                                    ) as EntityInventoryComponent;
                                                    switch (transferType) {
                                                        case 0:
            ...
            form4.textField("To Block", "dimension, x, y, z", callerPlayer.dimension.id + ", " + Math.floor(callerPlayer.location.x) + ", " + Math.floor(callerPlayer.location.y) + ", " + Math.floor(callerPlayer.location.z))*/
                                            ...
                                            form4
                                                ...
                                                .then((t) => {
                                                    if (t.canceled) return;
                                                    let [
                                                        transferType, blockSelectionMode, fromBlockPosition, playerTarget, debug2,
                                                    ] = t.formValues;
                                                    ...
                                                    let blockPositionC = blockPositionB.getComponent(
                                                        "inventory"
                                                    ) as BlockInventoryComponent;
                                                    let playerTargetB = Number(playerTarget);
                                                    const toInventory = players[playerTargetB].getComponent(
                                                        "inventory"
                                                    ) as EntityInventoryComponent;
                                                    if (Number(
                                                        blockSelectionMode
                ...
                targetList = String([String(targetList), players[index].nameTag]).split(",");
                                            ...
                                            form3
                                                ...
                                                .then((t) => {
                                                    if (t.canceled) return;
                                                    let [
                                                        transferType, inventoryRow, fromBlockSelectionMode, fromBlockPosition, toBlockSelectionMode, toBlockPosition, debug2,
                                                    ] = t.formValues;
                                                    let fromBlockPositionB = world
                                                        .getDimension(
                                                            String(
                                        ...
                                        case 1:
                                            ...
                                            form2
                                                ...
                                                .then((t) => {
                                                    if (t.canceled) return;
                                                    let [
                                                        transferType, inventoryRow, playerTarget, playerViewer, debug2,
                                                    ] = t.formValues;
                                                    let playerTargetB = Number(playerTarget);
                                                    let playerViewerB = Number(playerViewer);
                                                    const fromInventory = players[playerTargetB].getComponent(
    ...
    form.textField("Item Data", "Trigger Event", "");*/
                    ...
                    form.show(players[playerViewerB] as any)
                        .then((r) => {
                            ...
                            // This will assign every input their own variable
                            let [
                                itemName, itemLore, canDestroy, canPlaceOn, triggerEvent, amount, keepOnDeath, lockMode, setLore, clearLore, newItem, newItemType, newItemCount /*, newItemData*/, moveItem, moveFromSlot, moveToSlot, moveFromContainerType, moveFromContainer, moveFromContainerBlock, moveToContainerType, moveToContainer, moveToContainerBlock, swapItems, swapSlot, swapOtherSlot, swapContainerType, swapContainer, swapContainerBlock, swapOtherContainerType, swapOtherContainer, swapOtherContainerBlock, transferItem, transferFromSlot, transferFromContainerType, transferFromContainer, transferFromContainerBlock, transferToContainerType, transferToContainer, transferToContainerBlock, debug,
                            ] = r.formValues; /*
    console.warn(r.formValues);*/
    ...
    try{item.triggerEvent(String(triggerEvent));} catch(e){console.error(e, e.stack);}*/ //removed in 1.20.70.21
        ...
        console.warn([String(targetList)]);*/
            ...
            if (message.startsWith("players:") &&
                ...
                form2
                    .show(
                        playerList[playerList.findIndex((x) => x == sourceEntity)] as any
                    )
                    .then((t) => {
                        if (t.canceled) return;
                        let [playerTarget, playerViewer] = t.formValues;
                ...
                .then((to) => {
                    ...
                    elementValues.forEach((v, i) => {
                        switch (i % 5) {
                            case 0:
                                world.setDynamicProperty(
                                    `customUIElement:${formId}|${form.indexList[Math.floor(i / 5)]}`,
                                    `${customElementTypeIds[Number(elementValues[i])]}|${elementValues
                                        .slice(i + 1, i + 4)
                                        .join("|")}`
                                );
                                break;
        ...
        if (id == "andexdb:debugStickMenuC" ||
            ...
            form.show(
                ...
                .then((r) => {
                    ...
                    let [
                        includeLiquidBlocks, includePassableBlocks, maxDistance,
                    ] = r.formValues;
                    let blockPropertyValue2: any;
            ...
            try {
                if (block2.getComponent("fluid_container") != undefined) {
                    form.textField(
                        ...
                        `${block2.getComponent("fluid_container").fluidColor.red}, ${block2.getComponent("fluid_container").fluidColor
                                    ...
                                    block2.setPermutation(
                                        BlockPermutation.resolve(
                                            block2.typeId,
                                            {
                                                [String(
                                                    blockPropertyIdentifier
                                                ).split(", ")[0]]: blockPropertyValueArray[0],
                                            }
                                    ...
                                    block2.setPermutation(
                                        BlockPermutation.resolve(
                                            block2.typeId,
                                            {
                                                ...
                                                [String(
                                                    blockPropertyIdentifier
                                                ).split(", ")[1]]: blockPropertyValueArray[1],
                                            }
                                        )
                                    );
                                } catch (e) {
                                    ...
                                    block2.setPermutation(
                                        BlockPermutation.resolve(
                                            block2.typeId,
                                            {
                                                ...
                                                [String(
                                                    blockPropertyIdentifier
                                                ).split(", ")[2]]: blockPropertyValueArray[2],
                                            }
                                        )
                                    );
                                } catch (e) {
                                    ...
                                    block2.setPermutation(
                                        BlockPermutation.resolve(
                                            block2.typeId,
                                            {
                                                ...
                                                [String(
                                                    blockPropertyIdentifier
                                                ).split(", ")[3]]: blockPropertyValueArray[3],
                                            }
                                        )
                                    );
                                } catch (e) {
                                    ...
                                    block2.setPermutation(
                                        BlockPermutation.resolve(
                                            block2.typeId,
                                            {
                                                ...
                                                [String(
                                                    blockPropertyIdentifier
                                                ).split(", ")[6]]: blockPropertyValueArray[6],
                                            }
                                        )
                                    );
                                } catch (e) {
        ...
        if (id == "andexdb:debugScreen") {
            ...
            form.show(
                ...
                .then((r) => {
                    ...
                    let [
                        setType, blockPropertyIdentifier, blockPropertyValue, toggle,
                    ] = r.formValues;
      ...
      form.textField("Trigger Event", "Trigger Event")*/ /*
                    ...
                    if (Boolean(setWeather) == true && weatherDuration == "") {
                        try {
                            world
                                ...
                                .setWeather(
                                    weatherList3[weatherList2.indexOf(
                                        String(weatherType)
                                    )]
                                );
                        } catch (e) {
            ...
            console.warn(targetList);*/
                ...
                for (let i in playerName.slice(1)) {
                    switch (playerName[i].split(":")[0]) {
                        ...
                        case "triggerEvent":
                            try {
                                playerList[targetList.indexOf(String(playerName[0]))].triggerEvent(
        ...
        if (sourceType == "Entity") {message3 = Vector.add(sourceEntity.location, {x: Number(message2[1].split("~")[1]), y: Number(message2[1].split("~")[2]), z: Number(message2[1].split("~")[3])})} else { message3 = Vector.add(sourceBlock.location, {x: Number(message2[1].split("~")[1]), y: Number(message2[1].split("~")[2]), z: Number(message2[1].split("~")[3])}) }
            ...
            console.warn("RGB")*/
                            ...
                            if (String(message2[index]).startsWith(
                                "setColorRGBA:"
                            )) {
                                /*
            console.warn("RGBA")*/
