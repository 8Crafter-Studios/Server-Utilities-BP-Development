        ...
        if (securityVariables.ultraSecurityModeEnabled) {
            if (securityVariables.testPlayerForPermission(event.player, "andexdb.canUseEntityDebugSticks") == false) {
                event.player.sendMessage(
                    "§cYou do not have permission to use an Entity Debug Stick. You need the following permission to use this item: andexdb.canUseEntityDebugSticks"
                );
                return;
            }
        }
            ...
            "§btypeId§a: §u" +
                ...
                (!!playerTargetB.getComponent("projectile")
                    ? "§a, §bprojectile§a: §9{ §3airInertia§a: §c" +
                      ...
                      playerTargetB.getComponent("projectile")?.catchFireOnHurt +
                      "§a, §bprojectile§a: §3catchFireOnHurt§a: §g" +
                      playerTargetB.getComponent("projectile")?.critParticlesOnProjectileHurt
                    : "")
        );
    }
});
