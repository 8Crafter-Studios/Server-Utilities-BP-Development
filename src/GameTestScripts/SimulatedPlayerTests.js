...
GameTest.register("AndexdbTests", "spawn_simulated_player_other", (test) => {
  ...
  GameTest.register("AndexdbTests", "spawn_simulated_player_custom", (test) => {
    ...
    if(world.getDynamicProperty("andexdbGametest:spawnSimulatedPlayerLocation")?.x != undefined){spawnLocA = test.relativeLocation(world.getDynamicProperty("andexdbGametest:spawnSimulatedPlayerLocation"))}
    ...
    .thenExecuteAfter(0, () => {if (player.hasTag("customandexdbSimulatedPlayer:remove")) { test.removeSimulatedPlayer(player)} else { if (player.hasTag("customandexdbSimulatedPlayer:remove")) { player.respawn()}  else { if (player.hasTag("customandexdbSimulatedPlayer:disconnect")) { player.disconnect()}   else { if (player.hasTag("customandexdbSimulatedPlayer:fly")) { player.fly()}   else { if (player.hasTag("customandexdbSimulatedPlayer:stopFlying")) { player.stopFlying()} } } } }})
    .thenExecuteAfter(0, () => {
      test.assertEntityPresent("player", spawnLoc, false);
    })
    .thenExecuteAfter(2147483647, () => {
  ...
  GameTest.register("AndexdbTests", "spawn_without_behaviors", (test) => {
    ...
    if(world.getDynamicProperty("andexdbGametest:spawnWithoutBehaviorsLocation")?.x != undefined){locationA = world.getDynamicProperty("andexdbGametest:spawnWithoutBehaviorsLocation")}
    ...
    GameTest.register("AndexdbInternalTests", "spawn_simulated_player_custom_internal", (test) => {
      let spawnLocA = { x: 1, y: 5, z: 1 };
      if(world.getDynamicProperty("andexdbGametest:spawnSimulatedPlayerInternalLocation")?.x != undefined){spawnLocA = test.relativeLocation(world.getDynamicProperty("andexdbGametest:spawnSimulatedPlayerInternalLocation"))}
      const spawnLoc = spawnLocA;
      const landLoc = { x: 1, y: 2, z: 1 };
    ...
    GameTest.register("AndexdbInternalTests", "spawn_without_behaviors_internal", (test) => {
      let locationA = test.worldLocation({ x: 1, y: 5, z: 1})
      if(world.getDynamicProperty("andexdbGametest:spawnWithoutBehaviorsInternalLocation")?.x != undefined){locationA = world.getDynamicProperty("andexdbGametest:spawnWithoutBehaviorsInternalLocation")}
      const location = test.relativeLocation(locationA);
      ...
      GameTest.register("AndexdbInternalTests", "pulse_redstone_internal", (test) => {
        ...
        if(world.getDynamicProperty("andexdbGametest:pulseRedstoneInternalLocation")?.x != undefined){locationA = world.getDynamicProperty("andexdbGametest:pulseRedstoneInternalLocation")}
        const location = test.relativeLocation(locationA);
        ...
        GameTest.register("AndexdbInternalTests", "script_eval_internal", (test) => {
          try{eval(String(world.getDynamicProperty("andexdbGametest:scriptEvalInternalCode")))}catch(e){console.error(e, e.stack); test.fail(e, e.stack)}
          test.succeed()
        })
          .maxTicks(21474836)
          .structureName(String(world.getDynamicProperty("andexdbGametestSettigns:internalGametestStructureOverride") ?? "ComponentTests:platform"))
