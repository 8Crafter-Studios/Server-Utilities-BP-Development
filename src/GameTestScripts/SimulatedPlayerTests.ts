...
function targetSelectorAllListE(selector: string, position: string) {
        ...
        try {
            selectedEntity.push(
                world.scoreboard
                    ...
                    [i as `${number}`]?.participant.getEntity()!
            );
        } catch (e) {}
    }
    DimensionTypes.getAll().forEach((dt) => {
...
function isNear(n1: number, n2: number) {
...
GameTest.register("AndexdbInternalTests", "set_tnt_fuse_internal", (test) => {
    let tntEntities = targetSelectorAllListE(
        String(world.getDynamicProperty("andexdbGametest:setTntFuseInternalTargetSelector") ?? "@e [type=tnt,c=1]"),
        String(world.getDynamicProperty("andexdbGametest:setTntFuseInternalTargetSelectorExecutionLocation") ?? test.worldLocation({ x: 1, y: 2, z: 1 }))
    );
