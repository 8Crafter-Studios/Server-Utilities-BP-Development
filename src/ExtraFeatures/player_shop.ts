...
§r§eOptional Enchantments: ${
            ...
            form.slider(
                ...
                { valueStep: Math.min(item.step ?? 1, item.amountWanted), defaultValue: Math.min(item.step ?? 1, item.amountWanted) }
            );
            const r = await form.forceShow(player);
            if (r.canceled == true || (r.formValues?.[0] as number) == 0) {
                return 1;
...
export class PlayerShopManager {
    ...
    static async playerShopSystemSettings_main(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<0 | 1> {
        ...
        form.toggle(
            ...
            { defaultValue: config.shopSystem.player.allowSellingLockInSlotItems }
        );
        ...
        form.toggle(
            ...
            { defaultValue: config.shopSystem.player.allowSellingKeepOnDeathItems }
        );
        form.textField(
            ...
            "5",
            { defaultValue: config.shopSystem.player.maxShopsPerPlayer.toString() }
        );
    ...
    static async addPlayerShop(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<0 | 1> {
        ...
        form.textField(`§l§fButton Title§r§f\nThe title of the button for this shop.`, `${(sourceEntity as Player).name ?? sourceEntity.nameTag}'s Shop`, {
            defaultValue: `${(sourceEntity as Player).name ?? sourceEntity.nameTag}'s Shop`,
        });
        form.textField(
        ...
        form.textField(
            ...
            { defaultValue: `This is ${(sourceEntity as Player).name ?? sourceEntity.nameTag}'s shop.` }
        );
    ...
    static async managePlayerShop_settings(sourceEntitya: Entity | executeCommandPlayerW | Player, shop: PlayerShop): Promise<0 | 1> {
        ...
        if (config.system.debugMode) {
            ...
            form.textField(
                ...
                { defaultValue: JSON.stringify(shop.playerName).slice(1, -1).replaceAll('\\"', '"') }
            );
        }
        form.submitButton("Save");
        return (await form
