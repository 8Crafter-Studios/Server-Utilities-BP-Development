...
export class ServerShop {
    ...
    async openShop(
        player: Player,
        mode: "buy" | "sell" | "both" | "none" = this.sellShop && this.buyShop ? "both" : this.sellShop ? "sell" : this.buyShop ? "buy" : "both",
...
§6----------------------------------------------------${!!this.mainSellPageBodyText ? "\n§r" + this.mainSellPageBodyText : ""}`);
            data.forEach((v) => {
                form.button(
                    ...
                    (v.texture ?? "") === "" ? (v.type === "page" ? "textures/ui/arrow_right" : "loading") : v.texture
                );
            });
            if (showBackButton) {
                form.button(customFormUICodes.action.buttons.positions.title_bar_only + "Back", "textures/ui/arrow_left");
...
§6----------------------------------------------------${!!this.mainBuyPageBodyText ? "\n§r" + this.mainBuyPageBodyText : ""}`);
    ...
    async openShopPage<mode extends "buy" | "sell">(
        player: Player,
        data: mode extends "buy" ? BuyableShopElement[] : SellableShopElement[],
        path: [mode, ...string[]]
    ): Promise<0 | 1> {
        const mode = path[0];
        if (mode == "sell") {
            ...
            .then(async (r) => {
                ...
                switch (
                    ...
                    cullUndefined([
                        ...
                        "close",
                        "refresh",
                        ...(sourceEntity.hasTag("admin") ? (["manageShops", "serverShopSystemSettings"] as const) : ([] as const)),
...
export class LinkedServerShopCommands {
    ...
    static removeCommandsLinkedToShops(shopIDs: `shop:${string}`[]) {
        saveStringToDynamicProperties(
            JSON.stringify([...this.LinkedCommands.filter((c) => !shopIDs.includes(c[1]))]),
            "ServerShopSystem:LinkedServerShopCommands"
        );
    }
    ...
    static removeCommands(commands: string[]) {
        saveStringToDynamicProperties(
            JSON.stringify([...this.LinkedCommands.filter((c) => !commands.includes(c[0]))]),
            "ServerShopSystem:LinkedServerShopCommands"
        );
    }
...
export class ServerShopManager {
        ...
        return await form
            ...
            .then(async (r) => {
                ...
                switch ((["manageShops", "mainSettings", "shopItemSettings", "back", "close"] as const)[r.selection!]) {
                    ...
                    case "mainSettings":
                        if (((await ServerShopManager.serverShopSystemSettings_main(sourceEntity)) as any) !== 0) {
                            return await ServerShopManager.serverShopSystemSettings(sourceEntity);
                        } else {
                            return 0;
                        }
    ...
    static async addServerShop(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<1> {
        ...
        form.textField(
            ...
            "This is the main server shop.",
            { defaultValue: "This is the main server shop." }
        );
        ...
        form.toggle(
            `§l§fPublic Shop§r§f\nWhether or not this shop can be accessed by any player through the use of the \\viewservershops command, default is true`,
            { defaultValue: true }
        );
        form.submitButton("Save");
        return (await form
...
§rIs Sell Shop: ${shop.sellShop ? "§aTrue" : "§cFalse"}`);
        ...
        form.button(
            `${customFormUICodes.action.buttons.positions.main_only}${
                LinkedServerShopCommands.testShopHasLinkedCommand(shop.id) ? "Edit" : "Add"
            } Linked Command\n${
                LinkedServerShopCommands.testShopHasLinkedCommand(shop.id) ? LinkedServerShopCommands.LinkedCommands.find((c) => c[1] == shop.id) : "Not Set"
            }`,
            LinkedServerShopCommands.testShopHasLinkedCommand(shop.id) ? "textures/ui/book_edit_default" : "textures/ui/color_plus"
        );
        ...
        return await form
            ...
            .then(async (r) => {
                ...
                switch (response) {
                    ...
                    case 1:
                        if (
                            (await (LinkedServerShopCommands.testShopHasLinkedCommand(shop.id)
    ...
    static async manageServerShop_settings(sourceEntitya: Entity | executeCommandPlayerW | Player, shop: ServerShop): Promise<0 | 1> {
        ...
        form.toggle(
            `§l§fPublic Shop§r§f\nWhether or not this shop can be accessed by any player through the use of the \\viewservershops command, default is true`,
            { defaultValue: shop.publicShop ?? true }
        );
        form.submitButton("Save");
        return (await form
            .forceShow(sourceEntity as Player)
            .then(async (t) => {
                ...
                let [name, title, mainPageBodyText, buyShop, sellShop, publicShop] = t.formValues as [
                    name: string,
                    title: string,
                    mainPageBodyText: string,
                    buyShop: boolean,
    ...
    static async manageServerShop_addLinkedCommand(sourceEntitya: Entity | executeCommandPlayerW | Player, shop: ServerShop): Promise<0 | 1> {
        ...
        form.submitButton("Save");
        try {
            const t = await form.forceShow(sourceEntity as Player);
            if (t.canceled) {
                return 1;
            }
            let [command] = t.formValues as [command: string];
     ...
     * @param mode
     */
    static async manageServerShop_contents(
        ...
        const shopData =
            tryget(() => {
                return shop[(mode + "Data") as `${"buy" | "sell"}Data`] as (BuyableShopElement | SellableShopElement)[];
            }) ?? ([] as (BuyableShopElement | SellableShopElement)[]);
        shopData.forEach((s) => {
            form.button(
                customFormUICodes.action.buttons.positions.main_only + s.title,
                (s.texture ?? "") === "" ? (s.type === "page" ? "textures/ui/arrow_right" : "loading") : s.texture
            );
        });
                ...
                switch (response) {
                    case shopData.length:
                        ...
                        if (type == "pre-made") {
                            ...
                            let [title, texture, itemIndex, price, step, max] = r.formValues as [
                                title: string,
                                texture: string,
                                itemIndex: string,
                                price: string,
                        ...
                        if (
                            (shopData[response].type == "item"
                                ? await ServerShopManager.manageServerShop_manageItem(
                                      sourceEntity,
                                      shop,
                                      shopData[response] as SellableShopItem | ShopItem,
                                      response,
                                      mode
                                  )
                                : await ServerShopManager.manageServerShop_managePage(sourceEntity, shop, shopData[response] as ShopPage, response, mode)) == 1
                        ) {
                            return await ServerShopManager.manageServerShop_contents(sourceEntity, shop, mode);
                        }
                        return 0;
     ...
     * @returns
     */
    static async manageServerShop_manageItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        item: mode extends "buy" ? ShopItem : SellableShopItem,
        itemIndex: number,
        mode: mode
    ): Promise<0 | 1> {
        const sourceEntity = sourceEntitya instanceof executeCommandPlayerW ? sourceEntitya.player : sourceEntitya;
...
${mode == "buy" ? "Price" : "Value"}: ${mode == "buy" ? (item as ShopItem).price : (item as SellableShopItem).value}`
        ...
        return await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            switch (cullUndefined(["move", "edit", "delete", "applyTexturePreset", "back", "close"] as const)[r.selection!]) {
                ...
                case "applyTexturePreset": {
                    ...
                    if (r === 1) return await ServerShopManager.manageServerShop_manageItem(sourceEntity, shop, item, itemIndex, mode);
                    if (r === 0) return 0;
                    item.texture = r;
                    if (mode == "buy") {
                        let newData = shop.buyData;
     ...
     * @returns
     */
    static async manageServerShop_editItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        item: mode extends "buy" ? ShopItem : SellableShopItem,
        itemIndex: number,
        ...
        if (item.itemType == "newItemStack") {
            ...
            form.textField("Max Purchase Amount\n§oDefault is 64", "64", { defaultValue: String(item.max ?? 64) });
            form.textField(
                "Structure ID§c*§f\nThe ID of the 1x1x1 structure that contains the andexdb:saved_shop_item entity that has the saved item in its inventory slot.",
                "andexdbSavedShopItem:0",
                { defaultValue: JSON.stringify(item.structureID).slice(1, -1).replaceAll('\\"', '"') }
            );
            form.textField(
                "Entity ID§c*§f\nThe value of the andexdb:saved_shop_item_save_id dynamic property of the andexdb:saved_shop_item that has the saved item in its inventory slot.",
                "0",
                { defaultValue: JSON.stringify(item.entityID).slice(1, -1).replaceAll('\\"', '"') }
            );
        ...
        return (await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            if (item.itemType == "newItemStack") {
                let [title, texture, price, step, max, itemID, itemName, itemLore, canDestroy, canPlaceOn, lockMode, keepOnDeath] = r.formValues as [
                    title: string,
                    texture: string,
                    price: string,
                    step: string,
                ...
                let [title, texture, price, step, max, structureID, entityID] = r.formValues as [
                    title: string,
                    texture: string,
                    price: string,
                    step: string,
                ...
                let [title, texture, value, step, max, itemID] = r.formValues as [
                    title: string,
                    texture: string,
                    value: string,
                    step: string,
     ...
     * @returns
     */
    static async manageServerShop_addItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        type: "pre-made" | "pre-made_manual" | "newItemStack" | "giveCommand" | "sellable",
        mode: mode
    ): Promise<1> {
        const sourceEntity = sourceEntitya instanceof executeCommandPlayerW ? sourceEntitya.player : sourceEntitya;
        assertIsDefined(sourceEntity);
        ...
        if (type == "newItemStack") {
            ...
            form.textField("Max Purchase Amount\n§oDefault is 64", "64", { defaultValue: "64" });
            form.textField(
                "Item Type§c*\n§6NOTE: Items of this type (§bgiveCommand§6) must be available in commands. So things like minecraft:netherreactor will not work, to use those other item types use either the newItemStack type or the pre-made type.",
                "minecraft:stick"
            );
            ...
            form.textField(
                "Structure ID§c*§f\nThe ID of the 1x1x1 structure that contains the andexdb:saved_shop_item entity that has the saved item in its inventory slot.",
                "andexdbSavedShopItem:0",
                { defaultValue: "andexdbSavedShopItem:0" }
            );
            form.textField(
                "Entity ID§c*§f\nThe value of the andexdb:saved_shop_item_save_id dynamic property of the andexdb:saved_shop_item that has the saved item in its inventory slot.",
                "0",
        ...
        return (await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            if (type == "newItemStack") {
                ...
                let [title, texture, itemIndex, price, step, max, itemID, itemData] = r.formValues as [
                    title: string,
                    texture: string,
                    itemIndex: string,
                    price: string,
                ...
                let [title, texture, itemIndex, price, step, max, structureID, entityID] = r.formValues as [
                    title: string,
                    texture: string,
                    itemIndex: string,
                    price: string,
     ...
     * @returns
     */
    static async manageServerShop_managePage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        page: ShopPage,
        pageIndex: number,
...
Texture: ${page.texture}`
        ...
        return await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            switch (cullUndefined(["contents", "move", "edit", "delete", "applyTexturePreset", "back", "close"] as const)[r.selection!]) {
                ...
                case "applyTexturePreset": {
                    const r = await selectTexturePreset(sourceEntity);
                    if (r === 1) return await ServerShopManager.manageServerShop_managePage(sourceEntity, shop, page, pageIndex, mode);
                    if (r === 0) return 0;
                    page.texture = r;
                    if (mode == "buy") {
                        let newData = shop.buyData;
     ...
     * @returns
     */
    static async manageServerShop_editPage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        page: ShopPage,
        pageIndex: number,
     ...
     * @returns
     */
    static async manageServerShop_addPage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        mode: mode
    ): Promise<1> {
     ...
     * @returns
     */
    static async manageServerShopPage_contents<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]]
    ): Promise<0 | 1> {
        ...
        const mode = path[0];
        let form = new ActionFormData();
        const shopDataA: ShopPage =
            tryget(() => {
                return getPathInObject(shop[(mode + "Data") as `${"buy" | "sell"}Data`], path) as ShopPage;
            }) ?? ({} as ShopPage);
        const shopData: (BuyableShopElement | SellableShopElement)[] =
            tryget(() => {
                return shopDataA.data as (BuyableShopElement | SellableShopElement)[];
            }) ?? ([] as (BuyableShopElement | SellableShopElement)[]);
            ...
            default:
                if (
                    (shopData[response].type == "item"
                        ...
                        : await ServerShopManager.manageServerShopPage_managePage(
                              sourceEntity,
                              shop,
                              [...path, "data", String(response)],
                              shopData[response] as ShopPage,
                              response
                          )) == 1
                ) {
                    return await ServerShopManager.manageServerShopPage_contents(sourceEntity, shop, path);
     ...
     * @returns
     */
    static async manageServerShopPage_manageItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]],
        item: mode extends "buy" ? ShopItem : SellableShopItem,
...
${mode == "buy" ? "Price" : "Value"}: ${mode == "buy" ? (item as ShopItem).price : (item as SellableShopItem).value}`
        ...
        return await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            switch (cullUndefined(["move", "edit", "delete", "applyTexturePreset", "back", "close"] as const)[r.selection!]) {
                ...
                case "applyTexturePreset": {
                    const r = await selectTexturePreset(sourceEntity);
                    if (r === 1) return await ServerShopManager.manageServerShopPage_manageItem(sourceEntity, shop, path, item, itemIndex);
                    if (r === 0) return 0;
                    item.texture = r;
                    if (mode == "buy") {
                        let newData = shop.buyData;
     ...
     * @returns
     */
    static async manageServerShopPage_editItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]],
        item: mode extends "buy" ? ShopItem : SellableShopItem,
     ...
     * @returns
     */
    static async manageServerShopPage_addItem<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]],
        type: "pre-made" | "pre-made_manual" | "newItemStack" | "giveCommand" | "sellable"
     ...
     * @returns
     */
    static async manageServerShopPage_managePage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]],
        page: ShopPage,
...
Texture: ${page.texture}`
        ...
        return await form.forceShow(sourceEntity as Player).then(async (r) => {
            ...
            switch (cullUndefined(["contents", "move", "edit", "delete", "applyTexturePreset", "back", "close"] as const)[r.selection!]) {
                ...
                case "applyTexturePreset": {
                    const r = await selectTexturePreset(sourceEntity);
                    if (r === 1) return await ServerShopManager.manageServerShopPage_managePage(sourceEntity, shop, path, page, pageIndex);
                    if (r === 0) return 0;
                    page.texture = r;
                    if (mode == "buy") {
                        let newData = shop.buyData;
     ...
     * @returns
     */
    static async manageServerShopPage_editPage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]],
        page: ShopPage,
     ...
     * @returns
     */
    static async manageServerShopPage_addPage<mode extends "buy" | "sell">(
        sourceEntitya: Entity | executeCommandPlayerW | Player,
        shop: ServerShop,
        path: [mode, ...string[]]
    ): Promise<0 | 1> {
