    ...
    static async manageServerShops(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<0 | 1> {
        ...
        return await form.forceShow(sourceEntity as Player)
            .then(async (r) => {
                ...
                switch (
                    (!!shopsList[r.selection] ? "shop" : undefined) ?? (["newShop", "back", "close", "refresh"] as const)[
                        r.selection
                    ]
                ) {
                    case "newShop":
...
§rIs Sell Shop: ${shop.sellShop ? "§aTrue" : "§cFalse"}`);
    ...
    static async manageServerShop_editLinkedCommand(sourceEntitya: Entity | executeCommandPlayerW | Player, shop: ServerShop): Promise<0 | 1> {
        ...
        form.textField(
            ...
            "\\myshop",
            LinkedServerShopCommands.LinkedCommands.find((c) => c[1] == shop.id)[0]
        );
        form.submitButton("Save");
        return (await form.forceShow(sourceEntity as Player)
            .then((t) => {
    ...
    static async manageServerShop_editItem<mode extends "buy" | "sell">(
        ...
        if (item.itemType == "newItemStack") {
            ...
            form.textField(
                ...
                this.serverShopItemTextureHint,
            ...
            form.textField(
                ...
                "string",
                !!!item.itemName ? undefined : JSON.stringify(item.itemName).slice(1, -1).replaceAll('\\"', '"')
            );
            form.textField(
                "Item Lore\n§o(escape characters such as \\n are allowed)\n(set to [] to clear)",
                '["Line 1", "Line 2"...]',
                JSON.stringify(item.itemLore)
            );
            ...
            form.dropdown(
                ...
                [ItemLockMode.none, ItemLockMode.slot, ItemLockMode.inventory].indexOf(item.lockMode)
            );
            ...
            form.textField(
                ...
                JSON.stringify(item.structureID).slice(1, -1).replaceAll('\\"', '"')
            );
            form.textField(
                ...
                "0",
                JSON.stringify(item.entityID).slice(1, -1).replaceAll('\\"', '"')
            );
