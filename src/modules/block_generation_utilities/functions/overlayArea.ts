 ...
 */
export async function overlayArea<LiteModeEnabled extends boolean = false>(
    begin: Vector3,
    end: Vector3,
    dimension: Dimension,
    block: (location: DimensionLocation, index: bigint) => BlockPermutation,
    options?: OverlayAreaOptions<LiteModeEnabled>
): Promise<
                                ...
                                try {
                                    ...
                                    if (!(!b || b.y >= dimension.heightRange.max - 1 || b.y >= end.y || b.y < begin.y - 1)) {
                                        for (let y = b.y + 1; y < Math.min(b.y + 1 + (options?.layers ?? 1), dimension.heightRange.max, end.y + 1); y++) {
                                            if (y >= dimension.heightRange.max || y > end.y) break;
                                            dimension
                                                .getBlock({ x, y, z })
                                                ?.setPermutation(
                                                    block({ x, y, z, dimension }, index + (options?.pillarSequencePatternMode ? 0n : BigInt(b.y + 1 - y)))
                                                ); // Place the new block.
                                        }
                                    }
                                } catch (e) {
                                    ...
                                    if (options.blockMask.testIfMatches(b)) {
                                        if (Math.random() <= integrity / 100) {
                                            for (let y = b.y + 1; y < Math.min(b.y + 1 + (options?.layers ?? 1), dimension.heightRange.max, end.y + 1); y++) {
                                                ...
                                                const p = block(
                                                    { x, y, z, dimension },
                                                    index + (options?.pillarSequencePatternMode ? 0n : BigInt(b.y + 1 - y))
                                                );
                                                if (!p.matches(bA.typeId, bA.permutation.getAllStates())) {
                                                    // Make sure the block is diferent than the once that would be placed there before increasing the counter.
                                                    bA.setPermutation(p); // Place the new block.
