...
export async function generateNBTFileD(
    ...
    generateTickingAreaFillCoordinatesC(player.location, (()=>{let a = new CompoundBlockVolume(); a.pushVolume({volume: new BlockVolume(Object.assign(location, {y: 320}),Vector3Utils.add(location, Object.assign(Vector3Utils.scale(VECTOR3_ONE, 320), {y: 0})))}); return a})(), player.dimension).then(tac=>{*/ try {
        ...
        if (location.dimension.runCommand(
            `/tickingarea add ${location.x} -64 ${location.z} ${location.x + 136} 325 ${location.z + 136} "${id}"`
        ).successCount == 0) {
            player.sendMessage(
                "§6Warning: Map art generation may be incomplete because max amount of tickingareas were used up so some chunks might have not been loaded. "
            );
        }
        await waitTicks(2);
        try {
            switch (detectNBTDataType(nbt)) {
                ...
                case "nbt":
                    generateNBTFileF(
                        location,
                        parseJSONifiedNBTData(compressJavaNBTData(nbt))
                    );
                    break;
                case "bnbt":
                ...
                case "pnbt":
                    generateNBTFileE(
                        location,
                        compressBedrockNBTData(parseJSONifiedNBTData(nbt))
                    );
                    break;
                case "rawuenbt":
                    generateNBTFileE(
