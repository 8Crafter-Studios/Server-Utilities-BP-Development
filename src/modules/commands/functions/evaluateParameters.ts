        ...
        .forEach((p, i) => {
            switch (true) {
                case paramEval.trim() == "":
                    {
                        if (!!(p.type as string).match(
                            ...
                            argumentsa.push("");
                        } else if (!!(p.type as string).match(
                            /^f-[a-zA-Z0-9!@#$%^&*<>,.~]+$/
                        )) {
                            argumentsa.push(
                                Object.fromEntries(
                ...
                case p.type == "non-booleanString":
                    {
                        if (["true", "false", "t", "f", "1", "0"].includes(
                            paramEval
                                .trimStart()
                                .split(" ")[0]
                                .toLowerCase()
                        ...
                        if (!!paramEval
                            ...
                            paramEval =
                                paramEval
                                    .trimStart()
                                    .slice(
                                        paramEval.trimStart().indexOf(value) +
                                        value.length
                                    ) ?? "";
                            try {
                ...
                case p.type == "blockPattern":
                    {
                        ...
                        paramEval =
                            paramEval.slice(
                                paramEval.indexOf(ep.raw) + ep.raw.length
                            ) ?? "";
                        try {
                ...
                case p.type == "targetSelector":
                    {
                        if (!paramEval.trimStart().startsWith("@")) {
                            ...
                            if (!!paramEval
                                ...
                                paramEval =
                                    paramEval.slice(
                                        paramEval.indexOf(value) + value?.length
                                    ) ?? "";
                                try {
                                    argumentsa.push(value);
                                } catch (e) {
                ...
                case p.type == "Vectors":
                    {
                        let value = paramEval.match(
                            new RegExp(
                                String.raw`(?<!(?<!^([^"]*["][^"]*)+)(([^"]*(?<!([^\\])(\\\\)*?\\)"){2})*([^"]*(?<!([^\\])(\\\\)*?\\)")[^"]*)(((((?<=[\s\~\!\^\%\&\*\d])|^)[\~\!\^\%\&\*](?:[\-\+]?\d+(\.\d+)?)?)|(((?<=\s)|^)[\-\+]?\d+(\.\d+)?))\s*?){${p.vectorCount ?? 3}}(?!([^"]*(?<!([^\\])(\\\\)*?\\)")[^"]*(([^"]*(?<!([^\\])(\\\\)*?\\)"){2})*(?!([^"]*["][^"]*)+$))`
                            )
                        )?.[0];
                        paramEval =
                            paramEval.slice(
