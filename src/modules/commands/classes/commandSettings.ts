...
import type { RegisteredCommandSettings } from "init/classes/CommandRegistry";
...
export class commandSettings<T extends "built-in" | "custom" | "unknown" = "unknown"> {
    ...
    constructor(commandSettingsId: string, command?: command<T>) {
        this.type = (
            commandSettingsId.startsWith("built-inCommandSettings:")
                ? "built-in"
    ...
    get enabled() {
        return (this?.parsed?.enabled as boolean) ?? this?.defaultSettings?.enabled ?? true;
    }
    set enabled(enabled) {
        this.save({ enabled });
    }
    get requiredTags() {
        return (this?.parsed?.requiredTags as string[]) ?? this?.defaultSettings?.requiredTags ?? ["canUseChatCommands"];
    }
