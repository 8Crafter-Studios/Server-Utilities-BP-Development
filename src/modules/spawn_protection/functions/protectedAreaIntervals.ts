...
import { ProtectedAreaCategory, ProtectedAreas, ProtectedAreaTester, type AdvancedProtectedAreaCategory } from "init/variables/protectedAreaVariables";
 ...
 */
let zonesWithExecutableActions = ProtectedAreas.areas.advancedAreaCategories.filter(
    (category) =>
...
export function refreshZones() {
    zonesWithExecutableActions = ProtectedAreas.areas.advancedAreaCategories.filter(
        (category) =>
            category.enabled !== false &&
            ((category.noPVPZone !== false && category.noPVPZone !== undefined && category.noPVPZone.enabled !== false) ||
                (category.tagZone !== false && category.tagZone !== undefined && category.tagZone.enabled !== false))
    ...
    if (
        !!zonesWithExecutableActions.find(
            (category) =>
                category.tagZone !== false && category.tagZone !== undefined && category.tagZone.enabled !== false && category.tagZone.playersOnly === false
        )
    ) {
 ...
 */
async function zoneActionsInterval(): Promise<void> {
    if (zoneActionsIntervalActive) throw new Error("Zone actions interval is already active!");
...
async function executeZoneActions(players: Player[]) {
        ...
        zonesWithExecutableActions.forEach((category) => {
            ...
            if (category.noPVPZone !== false && category.noPVPZone !== undefined && category.noPVPZone.enabled !== false) {
                players.forEach((player) => {
                    if (
                        !player.getDynamicProperty("noPVPZoneActiveFromCustomAreaCategory:" + category.id) &&
                        ...
                        if (
                            player.hasTag("is_in_no_pvp_zone") &&
                            !player
                ...
                if (prop.playersOnly !== false) {
                    players.forEach((player) => {
                        if (
                            ...
                            try {
                                (JSON.parse(player.getDynamicProperty("tagZoneActiveFromCustomAreaCategory:" + category.id) as string) as string[]).forEach(
                                    (tag) => {
                                        if (
                                            player.hasTag(tag) &&
                                            !player
                    ...
                    entities.forEach((entity) => {
                        if (
                            ...
                            try {
                                (JSON.parse(entity.getDynamicProperty("tagZoneActiveFromCustomAreaCategory:" + category.id) as string) as string[]).forEach(
                                    (tag) => {
                                        if (
                                            entity.hasTag(tag) &&
                                            !entity
 ...
 */
function clearZoneTagsFromInactiveAreas(players: Player[]): void {
    players.map((player) =>
        player.getDynamicPropertyIds().forEach((id) => {
            if (id.startsWith("noPVPZoneActiveFromCustomAreaCategory:")) {
                ...
                if (category.noPVPZone !== false && category.noPVPZone !== undefined && category.noPVPZone.enabled !== false) return;
                player.setDynamicProperty(id);
                player.removeTag("is_in_no_pvp_zone");
                ...
                } else if (category.tagZone !== false && category.tagZone !== undefined && category.tagZone.enabled !== false) return;
                player.setDynamicProperty(id);
            }
        })
    );
 ...
 * @param players
 */
function testForRequiredZoneTagRemovals(players: Player[]) {
    players.map((player) => {
        let removeNoPVPZoneTag: boolean = false;
        let tagZoneTagsToRemove: string[] = [];
        let keepTagZoneTags: string[] = [];
        player.getDynamicPropertyIds().forEach((id) => {
            if (id.startsWith("noPVPZoneActiveFromCustomAreaCategory:")) {
                const category = ProtectedAreas.areas.advancedAreaCategories.find((category) => category.id === id.split(":")[1]);
                if (category === undefined) {
 ...
 * @returns {Promise<void>} A promise that resolves when the zone actions interval is stopped.
 */
export async function startZoneActionsInterval(): Promise<void> {
 ...
 */
export async function stopZoneActionsInterval(): Promise<boolean> {
