...
export function chatMessage(eventData: ChatSendBeforeEvent, bypassChatInputRequests = false) {
    ...
    let runreturn: boolean;
    runreturn = false;
    let returnBeforeChatSend: boolean;
    returnBeforeChatSend = false;
    let returnBeforeChatCommandsOrChatSend: boolean;
        ...
        world
            ...
            .forEach((p) => {
                try {
                    ...
                    srun(() =>
                        p.playSound(pn.getAllChatMessagesNotificationSound.soundId, {
    ...
    /*if(!((eventData.message.includes("${scripteval}") && (player.getDynamicProperty("canUseScriptEval") == true))||(eventData.message.includes("${run}") && ((player.isOp() == true)||(player.getDynamicProperty("canUseCommands") == true)))||(eventData.message.startsWith("\\")))){world.getDimension("overworld").runCommand("/playsound note.harp.ui @a ~~~ 1 0.75 1"); }*/ if (
        ...
        String(world.getDynamicProperty("andexdbSettings:validChatCommandPrefixes") ?? "")
            .split(", ")
            .forEach((prefix) => {
                if (newMessage.startsWith(prefix)) runreturn = true;
            });
    }
    ...
    if (
        ...
        !!!commanda
            ? config.invalidChatCommandAction == 2
                ...
                ? ((event.cancel = true),
                  player.sendMessage(
                      `§r§cUnknown command: ${
                          switchTest.startsWith("\\") ? "\\" + switchTest : switchTest
        ...
        if ((world.getDynamicProperty("andexdbSettings:disableCustomChatMessages") ?? false) != true) {
            if (
                world.getDynamicProperty("andexdbSettings:chatCommandsEnabled") != false &&
