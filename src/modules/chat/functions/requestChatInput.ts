...
export async function requestChatInput(player: Player, requestMessage?: string | RawMessage | (string | RawMessage)[]): Promise<string> {
    let id = idGenerator();
    ...
    return new Promise((resolve: (value: string) => void, reject: (error: Error) => void) => {
        function a() {
            ...
            if (!!!currentlyRequestedChatInput[player.id]!.anyInput[id]!.input) {
                ...
                let input = currentlyRequestedChatInput[player.id]!.anyInput[id]!.input as string;
