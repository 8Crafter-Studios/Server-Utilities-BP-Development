 ...
 */
export function findClosestColorMatch(rgb: [number, number, number], obj: { [key: string]: string }): string | null {
    ...
    Object.keys(obj).forEach((key: string) => {
        ...
        if (typeof color === "string" && color.startsWith("#")) {
            ...
            const distance: number = calculateColorDistance(rgb, hexRgb);
...
export function calculateColorDistance(rgb1: [number, number, number], rgb2: [number, number, number]): number {
    const rMean: number = (rgb1[0] + rgb2[0]) / 2;
    const r: number = rgb1[0] - rgb2[0];
    const g: number = rgb1[1] - rgb2[1];
    const b: number = rgb1[2] - rgb2[2];
