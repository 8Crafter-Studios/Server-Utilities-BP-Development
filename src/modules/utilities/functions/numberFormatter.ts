export function numberFormatter_compact(number: string | number | bigint | boolean, isMoney: boolean, decimalPlaces: number = 0){
    const dec = new (Decimal.clone({precision: 1000}))(String(number));
    const map = [
    ...
    if (found) {
        ...
        const formatted = (dec2.toFixed(found.threshold === 1n ? decimalPlaces : dec2.greaterThanOrEqualTo(10) ? 0 : 1)) + found.suffix;
        return (isMoney ? "$": "") + formatted;
    }
  
    return (isMoney ? "$": "") + dec.toFixed(decimalPlaces);
};

export function numberFormatter(
    number: string | number | bigint | boolean,
    options: { prefixWithDollarSign: boolean; addCommaSeparators: boolean },
