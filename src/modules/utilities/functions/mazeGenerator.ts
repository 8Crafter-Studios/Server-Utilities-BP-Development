import { BlockPermutation, type Dimension, Direction, Player, type Vector3 } from "@minecraft/server";
...
export async function mazeGenerator(startCorner: Vector3, endCorner: Vector3, dimension: Dimension, options: MazeOptions): Promise<void> {
    let {
        ...
        complexity = 1,
        airBlockType = BlockPermutation.resolve("air"),
        wallBlockType = BlockPermutation.resolve("stone"),
    } = options;
    ...
    const maze: Uint8Array = new Uint8Array(length * width);
    ...
    function* generateMaze(x: number, z: number): Generator<void> {
        const stack: [x: number, z: number][] = [[x, z]];
    ...
    if (entranceDirection === Direction.North) maze[startX] = 1;
    if (entranceDirection === Direction.South) maze[(length - 1) * width + startX] = 1;
    if (entranceDirection === Direction.West) maze[startZ * width] = 1;
    if (entranceDirection === Direction.East) maze[startZ * width + width - 1] = 1;
