...
/**
 * Gets the help page for a built-in command, with extra information.
 *
    ...
    if (cmd instanceof command) {
        return (!!!commanddescriptions[cmd.commandName as keyof typeof commanddescriptions] &&
            ...
            : `§e${cmd.commandName}${(cmd.aliases?.length ?? 0) != 0 ? ` (also ${cmd.aliases.map((v) => v.commandName).join(", ")})` : ""}:\n${
                  commanddescriptions[cmd.commandName as keyof typeof commanddescriptions] ?? cmd.settings.defaultSettings?.description ?? "Missing"
              }§r\nUsage:\n- ${(
              ...
              }${!!!cmd.settings.defaultSettings ? "" : "§r\nDefault Required Tags: " + JSONB.stringify(cmd.settings.defaultSettings.requiredTags)}${
                  !!!cmd.settings.requiredTags ? "" : "§r\nRequired Tags: " + JSON.stringify(cmd.settings.requiredTags)
              }${!!!cmd.settings.requiresOp ? "" : "§r\nRequires OP: " + JSON.stringify(cmd.settings.requiresOp)}${
                  !!!cmd.settings.requiredPermissionLevel ? "" : "§r\nRequired Permission Level: " + JSON.stringify(cmd.settings.requiredPermissionLevel)
        ...
        }${!cmd.command.categories ? "" : "§r\nCategories: " + JSON.stringify(cmd.command.categories)}${
            cmd.command.settings?.defaultSettings === undefined
                ? ""
                : "§r\nDefault Required Tags: " + JSONB.stringify(cmd.command.settings!.defaultSettings.requiredTags)
        }${cmd.command.settings?.requiredTags === undefined ? "" : "§r\nRequired Tags: " + JSON.stringify(cmd.command.settings!.requiredTags)}${
            cmd.command.settings?.requiresOp === undefined ? "" : "§r\nRequires OP: " + JSON.stringify(cmd.command.settings!.requiresOp)
        }${
            cmd.command.settings?.requiredPermissionLevel === undefined
                ? ""
                : "§r\nRequired Permission Level: " + JSON.stringify(cmd.command.settings!.requiredPermissionLevel)
