...
export function getCommandHelpPageExtra(commandName: string, player?: Player | executeCommandPlayerW | Entity) {
    ...
    return (!!!commanddescriptions[cmd.commandName as keyof typeof commanddescriptions] && !!!commandsyntaxes[cmd.commandName as keyof typeof commandsyntaxes] && !!!commandflags[cmd.commandName as keyof typeof commandflags] && !!!cmd.command_version) || cmd.isHidden
        ...
        : `§e${cmd.commandName}${(cmd.aliases?.length ?? 0) != 0 ? ` (also ${cmd.aliases.map((v) => v.commandName).join(", ")})` : ""}:\n${commanddescriptions[cmd.commandName as keyof typeof commanddescriptions] ?? cmd.settings.defaultSettings?.description ?? "Missing"}§r\nUsage:\n- ${(
            commandsyntaxes[cmd.currentCommandName as keyof typeof commandsyntaxes] ?? commandsyntaxes[cmd.commandName as keyof typeof commandsyntaxes]?.replaceAll((cmd.commandName.startsWith("\\\\") ? command.dp + cmd.commandName.slice(1) : command.dp + cmd.commandName) + " ", (cmd.currentCommandName.startsWith("\\\\") ? command.dp + cmd.currentCommandName.slice(1) : command.dp + cmd.currentCommandName) + " ") ?? tryget(() => typeof cmd.settings.defaultSettings.formats == "string" ? cmd.settings.defaultSettings.formats : cmd.settings.defaultSettings.formats instanceof Array ? cmd.settings.defaultSettings.formats["map"]((v: { format: any; }) => !!v?.format ? v.format : v).join(" ") : typeof cmd.settings.defaultSettings.formats.format === "string" ? cmd.settings.defaultSettings.formats.format : JSON.stringify(cmd.settings.defaultSettings.formats.format)) ?? "missing"
        )
            .split("\n")
