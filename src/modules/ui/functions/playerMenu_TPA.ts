...
export async function playerMenu_TPA(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<0 | 1> {
        ...
        .then(async (ra) => {
            ...
            switch ((["send", "outgoing", "incoming", "back", "close"] as const)[r.selection]) {
                case "send": {
                    ...
                    if (r.canceled || r.selection === playerslist.length) return await playerMenu_TPA(sourceEntity);
                    ...
                    try{
                        ...
                        if(e instanceof Error && e.message === "Duplicate Request"){
                            sourceEntity.sendMessage(
                                `§cYou have already sent a teleport request to ${target.name}.`
                            );
                        }else{
                            sourceEntity.sendMessage(
                                `§cAn error occured while sending a teleport request to ${target.name}: ${e}${e.stack}`
                            );
                        }
                    }
                    sourceEntity.sendMessage(
