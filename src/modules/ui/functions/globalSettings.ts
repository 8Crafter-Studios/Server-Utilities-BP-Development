...
export async function globalSettings(sourceEntitya: Entity | executeCommandPlayerW | Player) {
    ...
    form2.textField(
        ...
        "string",
        String(world.getDynamicProperty("andexdbSettings:chatCommandPrefix") ?? "\\")
    );
    form2.textField(
    ...
    form2.textField(
        ...
        "string",
        String(world.getDynamicProperty("andexdbSettings:chatRankPrefix") ?? "rank:")
    );
    form2.textField(
        "§l§fchatSudoPrefix§r§f\nPrefix for custom chat names, default is sudo:",
        "string",
        String(world.getDynamicProperty("andexdbSettings:chatSudoPrefix") ?? "sudo:")
    ); /*
    ...
    form2.dropdown(
        ...
        dimensionsd.indexOf(config.spawnCommandLocation.dimension.id as "minecraft:overworld" | "minecraft:nether" | "minecraft:the_end")
    );
    form2.textField(
        "§l§fpvpCooldownToTeleport§r§f\nHow long in seconds after getting damaged by another player that the player has to wait before they can teleport with commands such as \\spawn, \\home, \\gohome, \\tpa, and \\rtp, the default is 0",
        "x y z",
        config.pvpCooldownToTeleport.toString()
    );
    form2.dropdown(
