...
export function mapArtGeneratorB(sourceEntitya: Entity | executeCommandPlayerW | Player) {
    ...
    srun(() => {
        ...
        form.dropdown(
            ...
            { defaultValueIndex: dimensions.indexOf(sourceEntity.dimension) }
        );
        ...
        console.warn(r.formValues);*/
                ...
                if (String(snbt).includes("#")) {
                    (sourceEntity as Player).sendMessage("§6Warning: The snbt was censored! ");
                }
                ...
                generateNBTFileB(
                    Object.assign(
                        ...
                        {
                            dimension: dimensions[dimension as number] ?? sourceEntity.dimension,
                            y: (dimensions[dimension as number] ?? sourceEntity.dimension).heightRange.max - ((newsnbta.size[1] ?? 1) as number),
                        }
                    ),
                    newsnbta
                );
