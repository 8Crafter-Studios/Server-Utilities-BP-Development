...
export async function mainMenu(sourceEntity: loosePlayerType): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    while (true) {
        try {
            if (securityVariables.ultraSecurityModeEnabled) {
                if (securityVariables.testPlayerForPermission(player as Player, "andexdb.accessMainMenu") == false) {
                    const r = await showMessage(
                        player as Player,
                        "Access Denied (403)",
                        "You do not have permission to access this menu. You need the following permission to access this menu: andexdb.accessMainMenu",
                        "Back",
                        "Cancel"
                    );
            ...
            const buttons = (
                Object.entries(menuButtonIds.mainMenu.buttons) as [
                    keyof typeof menuButtonIds.mainMenu.buttons,
                    (typeof menuButtonIds.mainMenu.buttons)[keyof typeof menuButtonIds.mainMenu.buttons]
                ][]
            )
        ...
        case 4:
            ...
            break;*/
                ...
                case "manageCustomUIs":
                    customFormListSelectionMenu(player as Player);
                    return 0;
                case "moderation":
