...
export async function managePlayers(
    ...
    cachedPlayers?: [online: savedPlayer[], offline: savedPlayer[], banned: savedPlayer[]]
): Promise<0 | 1> {
    ...
    let displayPlayers: [online: savedPlayer[], offline: savedPlayer[], banned: savedPlayer[]] = cachedPlayers ?? [[], [], []];
    if (cachedPlayers === undefined) {
        ...
        displayPlayers = [
            ...
            savedPlayers.filter((_) => !_.isOnline && !_.isBanned).sort((a: savedPlayer, b: savedPlayer) => b.lastOnline - a.lastOnline),
            savedPlayers.filter((_) => !_.isOnline && _.isBanned).sort((a: savedPlayer, b: savedPlayer) => b.lastOnline - a.lastOnline),
        ] as [online: savedPlayer[], offline: savedPlayer[], banned: savedPlayer[]];
    }
    ...
    form.title(
        `${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Manage Players"} ${Math.min(
            numsavedplayers,
