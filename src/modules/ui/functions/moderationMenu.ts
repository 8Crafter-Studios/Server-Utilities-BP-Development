 ...
 */
export async function moderationMenu(sourceEntity: loosePlayerType): Promise<-2 | 0 | 1> {
    ...
    while (true) {
        try {
            ...
            form.button(
                customFormUICodes.action.buttons.positions.main_only + customFormUICodes.action.buttons.options.disabled + "§4Anti-Cheat\n§f(§cComing Soon!§f)",
            ...
            switch ((["quickActions", "manageBans", "antiSpam", "anticheat", "bannedItems", "back", "close"] as const)[response]) {
                case "quickActions":
                ...
                case "anticheat": {
                    ...
                    const r = await showMessage(player as Player, undefined, "§cSorry, the anticheat system does not exist yet.", "Back", "Close")
                    if (r.selection !== 1) {
                        continue;
                    } else {
                        return 0;
                ...
                case "bannedItems": {
                    ...
                    const r = await showMessage(player as Player, undefined, "§cSorry, the banned items system does not exist yet.", "Back", "Close")
                    if (r.selection !== 1) {
                        continue;
                    } else {
                        return 0;
