...
export async function uiSettings_menuConfigurations_playerMenu_editButtons_addRemovedButtons(sourceEntitya: Entity | executeCommandPlayerW | Player): Promise<0 | 1> {
    ...
    try{
        ...
        const buttons = (Object.entries(menuButtonIds.playerMenu.buttons) as [keyof typeof menuButtonIds.playerMenu.buttons, typeof menuButtonIds.playerMenu.buttons[keyof typeof menuButtonIds.playerMenu.buttons]][]).filter(b=>!currentButtonsA.includes(b[0])).sort((a, b)=>a[1].defaultButtonIndex - b[1].defaultButtonIndex);
...
Default Button Index: ${button.defaultButtonIndex}${button.deprecated ? "\n§pThis button is deprecated.§r" : ""}${
        ...
        if(rb.canceled || rb.selection === 1) return await uiSettings_menuConfigurations_playerMenu_editButtons_addRemovedButtons(sourceEntity);
