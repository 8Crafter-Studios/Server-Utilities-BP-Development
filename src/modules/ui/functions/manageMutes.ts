 ...
 */
export async function manageMutes(
    ...
    while (true) {
        ...
        if (cachedEntries === undefined) {
            ...
            displayEntries = Object.keys(ModerationActions.getAllMutes())
                .map((v) => [v, onlinePlayerNames.includes(v) ? "online" : "offline"] as [playerName: string, status: "online" | "offline"])
                .filter((v) =>
                    !!search ? (search.caseSensitive == true ? v[0].includes(search.value) : v[0].toLowerCase().includes(search.value.toLowerCase())) : true
                )
                .sort((a, b) => {
        ...
        form.title(
            `${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Manage Mutes"} ${Math.min(
                numentries,
 ...
 * @throws {TypeError} If sourceEntity is not an instance of the Player class or an instance of the executeCommandPlayerW class with a Player linked to it.
 */
export async function manageMute(sourceEntity: loosePlayerType, mute: [playerName: string, mute: muteData]): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    while (true) {
        try {
            let form = new ActionFormData();
            ...
            form.body(
                `§bPlayer Name: §a${mute[0]}\n§r§bMute Duration: §q${
                    ...
                    formatDateTime(new Date(mute[1].muteDate), timeZone) + " UTC" + (timeZone > 0 || Object.is(timeZone, 0) ? "+" : "") + timeZone
                }${
                    isPermanent
                        ? ""
                        : `\n§r§bUnmute Date: §q${
...
export async function addMute(sourceEntity: loosePlayerType): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    ...
    while (true) {
        try {
            ...
            form.textField(
                "Mute Duration (Time String, ex. 5y 7mo 6d 5h 3m 1s 17ms)\nLeave blank to make the mute duration permanent.",
                "permanent",
                defaultMuteDuration
            );
                ...
                if (
                    (
                        await showMessage(
                            ...
                            "The mute duration you entered is invalid. Please try again.\nHere is an example: 5y 7mo 6d 5h 3m 1s 17ms.",
                            "Back",
                            "Cancel"
                        )
                    ).selection !== 1
...
export async function addMuteOnPlayer(sourceEntity: loosePlayerType, targetName: string): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    let defaultMuteDuration = "";
    let defaultReason = "";
    while (true) {
