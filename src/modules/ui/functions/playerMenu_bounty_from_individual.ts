...
export async function playerMenu_bounty_from_individual(
    ...
    if (!target) throw new ReferenceError("[playerMenu_bounty_from_individual] No target player found.");
...
§aEmail: §b8crafteryt@gmail.com`,
                        ...
                        const r = await showMessage(
                            sourceEntity as Player,
                            "An Error occurred",
                            `An error occurred while canceling the bounty on ${target.name}. The following error was triggered: §c${e}${e?.stack}`,
                            "Back",
                            "Cancel"
                        );
                        if (r.canceled || r.selection == 0) {
