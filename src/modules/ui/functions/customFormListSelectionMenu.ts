...
export function customFormListSelectionMenu(player: Player) {
    ...
    forceShow(form1234, player).then((t) => {
        let ta = t as ActionFormResponse;
        if (ta.canceled &&
        ...
        switch (true) {
            ...
            default:
                forceShow(form123456, player).then((v) => {
                    let va = v as ActionFormResponse;
                    if (va.canceled &&
                    ...
                    switch (va.selection) {
                        ...
                        case 3:
                            ...
                            form12345678.body(
                                `Are you sure you want to delete the custom UI ${a[ta.selection]}`
                            );
                            form12345678.button1("Cancel");
                            form12345678.button2("Confirm");
                            forceShow(form12345678, player).then((u) => {
                                let ua = u as MessageFormResponse;
                                if (ua.canceled &&
