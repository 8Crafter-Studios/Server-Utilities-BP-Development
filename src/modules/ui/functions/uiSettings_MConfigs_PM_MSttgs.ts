...
/**
 * Displays and handles the main settings form for the player menu for a given entity.
 *
 * @async
 ...
 */
export async function uiSettings_menuConfigurations_playerMenu_mainSettings(sourceEntity: loosePlayerType): Promise<1 | 0> {
    ...
    try {
        ...
        form.title(customFormUICodes.modal.titles.formStyles.medium + "Player Menu Settings");
        const formOptionsMap = {
            enabled: () => form.toggle("§l§fEnabled§r§f\nWhether or not the player menu is enabled. Defaults to true.", { defaultValue: menuConfig.enabled }),
            showDeprecatedButtons: () =>
            ...
            itemName: () =>
                form.textField('§l§fPlayer Menu Item Name§r§f\nThe item name for the item that opens the player menu. Defaults to "Menu".', "item name", {
                    defaultValue: menuConfig.itemName,
                }),
