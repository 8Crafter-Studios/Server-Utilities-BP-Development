...
export async function playerMenu_bounties_list(
    ...
    let displayBounties: [totalBounty: TotalBounty, player: savedPlayer][] = cachedBounties ?? [];
    if (cachedBounties === undefined) {
        let bounties = TotalBounty.getAll()
            .map((b) => [b, savedPlayer.getSavedPlayer("player:" + b.targetId)] as [TotalBounty, savedPlayer])
        ...
        displayBounties = bounties
            ...
            .sort((a: [TotalBounty, savedPlayer], b: [TotalBounty, savedPlayer]) =>
                a[0].totalValue > b[0].totalValue ? -1 : a[0].totalValue < b[0].totalValue ? 1 : a[1].name > b[1].name ? -1 : a[1].name < b[1].name ? 1 : 0
            );
    }
    const numsavedplayers = displayBounties.length;
    form.title(
        `${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Bounties"} ${Math.min(
            numsavedplayers,
