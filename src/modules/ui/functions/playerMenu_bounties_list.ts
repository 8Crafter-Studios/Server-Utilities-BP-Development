...
export async function playerMenu_bounties_list(
    ...
    let bounties = TotalBounty.getAll().map(b=>[b, savedPlayer.getSavedPlayer("player:" + b.targetId)] as const)
    ...
    .filter((p) => !!search
        ...
        : true
    ).sort(((a: [TotalBounty, savedPlayer], b: [TotalBounty, savedPlayer]) => (a[0].totalValue > b[0].totalValue ? -1 : a[0].totalValue < b[0].totalValue ? 1 : a[1].name > b[1].name ? -1 : a[1].name < b[1].name ? 1 : 0)));
    const numsavedplayers = displayPlayers.length;
    form.title(
        `${!!search ? "Search Results" : "Bounties"} ${Math.min(
            numsavedplayers,
