...
export async function playerMenu_bounty_new(
    ...
    let displayPlayers: savedPlayer[] = cachedPlayers ?? [];
    ...
    form.title(
        `${customFormUICodes.action.titles.formStyles.gridMenu}${!!search ? "Search Results" : "Select Player"} ${Math.min(numsavedplayers, page * maxplayersperpage + 1)}-${Math.min(
            numsavedplayers,
...
§r§lAmount§r
Please enter the amount of money you would like to place on the bounty below.`,
                            "Money Amount"
                        )
                        ...
                        return (
                            (
                                await showMessage(
                                    sourceEntity,
                                    "Invalid Money Amount",
                                    "You may not place a negative value bounty, as that would steal someone's money.",
                                    "Back",
                                    "Close"
                                )
                            ).selection !== 1
                    ...
                    if (amount === 0n) {
                        return (
                            (await showMessage(sourceEntity, "Invalid Money Amount", "You may not place a $0 bounty.", "Back", "Close")).selection !== 1
                        ).toNumber();
                    }
                        ...
                        (
                            (
                                await showMessage(
                                    sourceEntity,
                                    "Are you sure?",
                                    `Are you sure you want to place a ${numberFormatter(amount, {
                                        addCommaSeparators: true,
                                        currencyPrefix: config.ui.menus.playerMenu_leaderboards.builtInStats.money.displayOptions.currencyPrefix,
                                    })} bounty on ${player.name}`,
                                    "Cancel",
                                    "Confirm"
                                )
                            ).selection === 1
                        ...
                        return (
                            (
                                await showMessage(
                                    sourceEntity,
                                    "Bounty Placed",
                                    `Successfully placed a ${numberFormatter(amount, {
                                        addCommaSeparators: true,
                                        currencyPrefix: config.ui.menus.playerMenu_leaderboards.builtInStats.money.displayOptions.currencyPrefix,
                                    })} bounty on ${player.name}.`,
                                    "Okay",
                        ...
                        return (
                            (
                                await showMessage(
                                    sourceEntity,
                                    "Bounty Canceled",
                                    `Your ${numberFormatter(amount, {
                                        addCommaSeparators: true,
                                        currencyPrefix: config.ui.menus.playerMenu_leaderboards.builtInStats.money.displayOptions.currencyPrefix,
                                    })} bounty on ${player.name} has been canceled.`,
                                    "Okay",
                                    "Close"
                                )
                            ).selection !== 1
