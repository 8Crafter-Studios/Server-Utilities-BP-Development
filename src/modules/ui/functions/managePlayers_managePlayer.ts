 ...
 */
export async function managePlayers_managePlayer(sourceEntity: loosePlayerType, targetPlayer: savedPlayer): Promise<0 | 1> {
    ...
    while (true) {
        try {
            ...
            form.body(
                ...
                }${ban.testForIdBannedPlayer(targetPlayer) ? "ID BANNED" : ban.testForIdBannedPlayer(targetPlayer) ? "NAME BANNED" : ""}`
            );
            ...
            if (copyInventoryToChestIsAvailable) {
                ...
                form.button(
                    `${customFormUICodes.action.buttons.positions.main_only}${
                        ...
                        copyInventoryToChestIsAvailable_formatVersion
                    ...
                    if (securityVariables.ultraSecurityModeEnabled) {
                        if (securityVariables.testPlayerForPermission(player, "andexdb.UIs.managePlayersUI.deleteSavedPlayerData") == false) {
                            const r = await showMessage(
                                player,
                                "Access Denied (403)",
                                "You do not have permission to access this menu. You need the following permission to access this menu: andexdb.UIs.managePlayersUI.deleteSavedPlayerData",
                                "Okay",
                                "Cancel"
                            );
                            if (r.canceled || r.selection == 0) {
                            ...
                            if (
                                (
                                    await showMessage(
                                        ...
                                        "Sorry, the copy inventory to chest feature is not available for player data format versions below 1.5.0.",
                                        "Back",
                                        "Close"
                                    )
                                ).selection !== 1
                            ...
                            if (
                                (
                                    await showMessage(
                                        player,
                                        "Feature Not Available.",
                                        "Sorry, the copy inventory to chest feature is not available for player inventory data saved with the §bconfig.system.useLegacyPlayerInventoryDataSaveSystem§r option set to true.",
                                        "Back",
                                        "Close"
                                    )
                                ).selection !== 1
                            ...
                            if (
                                (
                                    await showMessage(
                                        player,
                                        "Feature Not Available.",
                                        "Sorry, the copy inventory to chest feature is not available as this player's inventory save data is missing, this could be because §bconfig.system.autoSavePlayerData§r was set to false when the player's data was last saved.",
                                        "Back",
                                        "Close"
                                    )
                                ).selection !== 1
                            ...
                            if (
                                (
                                    await showMessage(
                                        player,
                                        "Feature Not Available.",
                                        "Sorry, the copy inventory to chest feature is not available for an unknown reason.",
                                        "Back",
                                        "Close"
                                    )
                                ).selection !== 1
                    ...
                    if (
                        (
                            await showMessage(
                                player,
                                undefined,
                                "§cSorry, the feature to manage hotbar presets for other players does not exist yet.",
                                "Back",
                                "Close"
                            )
                        ).selection !== 1
                    ...
                    if (
                        (
                            await showMessage(
                                player,
                                undefined,
                                "§cSorry, the feature to manage private warps for other players does not exist yet.",
                                "Back",
                                "Close"
                            )
                        ).selection !== 1
...
export async function managePlayers_managePlayer_viewData(sourceEntity: loosePlayerType, targetPlayer: savedPlayer): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    try {
        let form = new ActionFormData();
...
export async function managePlayers_managePlayer_managePermissions(sourceEntity: loosePlayerType, targetPlayer: savedPlayer): Promise<0 | 1> {
    const player = extractPlayerFromLooseEntityType(sourceEntity);
    try {
        ...
        form.toggle(
            `canUseChatCommands${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "canUseChatCommands")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "canUseChatCommands"
                              ) as SavedPlayerOnJoinAction_set_permission<"canUseChatCommands">
                          ).value
        ...
        form.toggle(
            `canUseDangerousCommands${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "canUseDangerousCommands")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "canUseDangerousCommands"
                              ) as SavedPlayerOnJoinAction_set_permission<"canUseDangerousCommands">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.toggle(
            `canUseScriptEval${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "canUseScriptEval")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "canUseScriptEval"
                              ) as SavedPlayerOnJoinAction_set_permission<"canUseScriptEval">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.toggle(
            `canUseCommands${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "canUseCommands")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "canUseCommands"
                              ) as SavedPlayerOnJoinAction_set_permission<"canUseCommands">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.toggle(
            `canBypassProtectedAreas${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "canBypassProtectedAreas")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "canBypassProtectedAreas"
                              ) as SavedPlayerOnJoinAction_set_permission<"canBypassProtectedAreas">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.toggle(
            `getAllChatCommands${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "getAllChatCommands")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "getAllChatCommands"
                              ) as SavedPlayerOnJoinAction_set_permission<"getAllChatCommands">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.toggle(
            `admin${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "admin")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "admin"
                              ) as SavedPlayerOnJoinAction_set_permission<"admin">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
        ...
        form.slider(
            `permissionLevel${
                !!targetPlayer.onJoinActions.find((a) => a.type == "set_permission" && a.permission == "permissionLevel")
                    ? `§a (Will be set to §b${
                          (
                              targetPlayer.onJoinActions.find(
                                  (a) => a.type == "set_permission" && a.permission == "permissionLevel"
                              ) as SavedPlayerOnJoinAction_set_permission<"permissionLevel">
                          ).value
                      }§a when the player joins)`
                    : ""
            }`,
            ...
            if (
                (
                    await new MessageFormData()
                        .body(
                            'Changing a player\'s permissions while they are offline will schedule the permission changes to be applied to the player when they join. Saving these changes will also result in any previously scheduled permission changes being canceled. To cancel the scheduled changes and keep the current permissions, just open the "Manage Permissions" menu again and save it without changing any options.'
                        )
