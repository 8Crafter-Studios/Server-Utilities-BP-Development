...
export async function securitySettings(
    ...
    if (securityVariables.ultraSecurityModeEnabled) {
        if(securityVariables.testPlayerForPermission(sourceEntity as Player, "andexdb.accessSecuritySettings") == false){
            const r = await showMessage(sourceEntity as Player, "Access Denied (403)", "You do not have permission to access this menu. You need the following permission to access this menu: andexdb.accessSecuritySettings", "Go Back", "Close");
...
export async function securitySettings_settingsSelection(
    ...
    if(securityVariables.ultraSecurityModeEnabled){
        form.button(
            "Ultra Security Mode Settings",
            "textures/ui/icon_setting"
        );
        form.button(
            "Ultra Security Mode Debug",
            "textures/ui/icon_setting"
        );
    };
    form.button("Back", "textures/ui/arrow_left");
...
form.button("Debug Screen", "textures/ui/ui_debug_glyph_color");*/
    ...
    return await forceShow(form, sourceEntity as Player)
        .then(async (ra) => {
            ...
            switch (response) {
                case 0:
                    if((await selectSecurityMode(sourceEntity as Player))==1){
                ...
                case 3:
                    if(securityVariables.ultraSecurityModeEnabled){
                        if((await ultraSecurityModeDebug(sourceEntity as Player))==1){
                            return await securitySettings_settingsSelection(sourceEntity)
                        }else{
                            return 0;
                        }
                ...
                case 0:
                    {
                        ...
                        if (!!!rb || rb?.canceled == true) {
                            return await securitySettings_playersWithPermissions_permission(
                                sourceEntity,
                                permission,
                                page,
                                maxplayersperpage,
...
export async function securitySettings_playersWithPermissions_UltraSecurityMode(
    ...
    if (securityVariables.ultraSecurityModeEnabled) {
        if(securityVariables.testPlayerForPermission(sourceEntity as Player, "andexdb.accessManagePlayersUI") == false){
            const r = await showMessage(sourceEntity as Player, "Access Denied (403)", "You do not have permission to access this menu. You need the following permission to access this menu: andexdb.accessManagePlayersUI", "Go Back", "Close");
            if(r.canceled || r.selection == 0){
                return 1;
            }else{
                return 0;
        ...
        .then(async (ra) => {
            ...
            switch (response) {
                ...
                case Object.keys(permissionType).length + 2:
                    ...
                    break;
                default:
                    if((await securitySettings_playersWithPermissions_permission_UltraSecurityMode(sourceEntity, Object.keys(permissionType).sort()[r.selection] as permissionType))==1){
                        return await securitySettings_playersWithPermissions_UltraSecurityMode(sourceEntity)
                    };
                    break;
            }
