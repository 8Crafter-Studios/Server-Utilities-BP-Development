...
export function fillBlocksHO(
    ...
    mainArray.push(
        new BlockVolume(
            { x: to.x + (to.x > from.x ? -1 : 1), y: from.y, z: from.z },
            { x: from.x + (from.x > to.x ? -1 : 1), y: from.y, z: from.z }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: to.x + (to.x > from.x ? -1 : 1), y: to.y, z: from.z },
            { x: from.x + (from.x > to.x ? -1 : 1), y: to.y, z: from.z }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: to.x + (to.x > from.x ? -1 : 1), y: from.y, z: to.z },
            { x: from.x + (from.x > to.x ? -1 : 1), y: from.y, z: to.z }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: to.x + (to.x > from.x ? -1 : 1), y: to.y, z: to.z },
            { x: from.x + (from.x > to.x ? -1 : 1), y: to.y, z: to.z }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: to.x, y: from.y, z: to.z + (to.z > from.z ? -1 : 1) },
            { x: to.x, y: from.y, z: from.z + (from.z > to.z ? -1 : 1) }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: to.x, y: to.y, z: to.z + (to.z > from.z ? -1 : 1) },
            { x: to.x, y: to.y, z: from.z + (from.z > to.z ? -1 : 1) }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: from.x, y: from.y, z: to.z + (to.z > from.z ? -1 : 1) },
            { x: from.x, y: from.y, z: from.z + (from.z > to.z ? -1 : 1) }
        )
    );
    mainArray.push(
        new BlockVolume(
            { x: from.x, y: to.y, z: to.z + (to.z > from.z ? -1 : 1) },
            { x: from.x, y: to.y, z: from.z + (from.z > to.z ? -1 : 1) }
        )
    );
    let counter = 0;
