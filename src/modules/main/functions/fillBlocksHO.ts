...
import type { fillBlocksHOTG } from "./fillBlocksHOTG";
...
export function fillBlocksHO(
    ...
    mainArray.push(
        new BlockVolume({ x: to.x + (to.x > from.x ? -1 : 1), y: from.y, z: from.z }, { x: from.x + (from.x > to.x ? -1 : 1), y: from.y, z: from.z })
    );
    ...
    mainArray.push(
        new BlockVolume({ x: from.x, y: from.y, z: to.z + (to.z > from.z ? -1 : 1) }, { x: from.x, y: from.y, z: from.z + (from.z > to.z ? -1 : 1) })
    );
        ...
        mainArray.forEach((v) => {
            try {
                counter += dimension.runCommand(
                    `fill ${v.from.x} ${v.from.y} ${v.from.z} ${v.to.x} ${v.to.y} ${v.to.z} ${placeholderid ?? "andexdb:ifill_command_placeholder_block"} ${
                        !!options?.matchingBlock ? "replace " + (options?.matchingBlock ?? "") : ""
                    } ${
