import { type Vector3, Dimension, BlockPermutation, BlockType, BlockVolume } from "@minecraft/server";

/**
 * @deprecated
 */
export function fillBlocksF(
    from: Vector3,
    to: Vector3,
    dimension: Dimension,
    block: string | BlockPermutation | BlockType,
    options?: { matchingBlock?: BlockPermutation | BlockType | string; }
) {
    ...
    let counter = 0;
    mainArray.forEach(
        (v) => (counter += dimension.runCommand(
            `fill ${v.from.x} ${v.from.y} ${v.from.z} ${v.to.x} ${v.to.y} ${v.to.z} ${block instanceof BlockPermutation
                ? block.type.id
                ...
                ? options?.matchingBlock instanceof BlockPermutation
                    ? "[" +
                    Object.entries(
                        options?.matchingBlock.getAllStates()
                    )
                        .map(
                            (v) => '"' +
                                v[0] +
